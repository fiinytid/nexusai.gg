...
const ICONS = [
  ...
  "Coin 84697600263846",        "Cash 70565105539676",
  "Diamond 75581768563141",     "Crystal 73150429062000",
  "Robux 113823942453285",      "Ticket 123370754779214",
  "Premium 78918235954057",     "VIP 97092630460629",
  "Sword 94091032987086",       "Shield 93114601642790",
  "Axe 75127143522091",         "Potion 71202349341308",
  "Chest 76137715921998",       "Crown 78843852703854",
  "Trophy 77830885604568",      "Key 96066489256923",
  "Bomb 96872034340553",        "Backpack 118915534669949",
  "Box 99990137483704",         "Book 117316658726625",
  "Egg 113316632422703",        "Hammer 95064026158349",
  "Shovel 84998465111718",      "Fire 73214946386499",
  "House 101953044632807",      "Settings 119570973950437",
  "Shopping Cart 123838677183783", "Stats 92574857197960",
  "Trash 72745454842879",       "Chat 94298126681415",
  "Checkmark 128850290702187",  "Close 109798318511632",
  "Info 119677199991519",       "Plus 127726919558379",
  "Minus 115333097448632",      "Warning 122437442880819",
  "Player 99097554161865",      "Friend 87070401810152",
  "Add Player 121328279027494", "Skull 126528254643859",
  "Ingot 83606937519307",       "Balloon 86067946513885",
  "Dog 94785235613863",         "Cat 136373929646470",
  "Bunny 97628616133746",       "Aura 103015582536746",
  "Trail 90501824327853",       "Angel Heart 77354444720914",
  "Leaf 122842695290895",       "Cloud 104293709713395",
  "Apple 120786616810420",
] as const;

const SOUNDS = [
  "Button Click (Modern) 6895079853", "Button Click (Light) 9114221199",
  "Menu Open 2550663487",             "Notif Success 2865227271",
  "Notif Error 5543666504",           "Sword Slash 12222229",
  "Hit Impact 131237241",             "Explosion 12222084",
  "Pistol Shot 5238260384",           "Gun Reload 131070682",
  "Jump 12222208",                    "Landing 12222152",
  "Footstep Floor 1156535269",        "Footstep Grass 132170343",
  "Teleport/Magic 138090544",         "Coin Collect 5153205307",
  "Item Pickup 2373079087",           "Level Up/Victory 2125193951",
  "Chest Open 1133314051",            "Rain & Thunder 151679162",
...
export function buildSysPrompt(ctx: SysPromptContext = {}): string {
  ...
  const header = [
    ...
    `Studio: ${connected ? "CONNECTED" : "OFFLINE"} | PlayTest: ${
      ptEnabled ? `ENABLED (${ptDur}s)` : "DISABLED"
    }`,
...
Never write asset IDs in prose — use words ("a star icon"). IDs go in action JSON properties only.`;

  // ── 3. Response Format ──────────────────────────────────────────────────────
  const responseFormat = `\
## RESPONSE FORMAT
- Plain text for explanations and confirmations.
- \`\`\`json  — Studio actions (single or batched).
- \`\`\`lua   — Reference-only code snippets.
- \`\`\`clarify — Divergent options: {"question":"...?","options":["A","B"]} (2–6 choices, use sparingly).
- Raw unfenced Lua pasted by user = their existing code; do not echo unless asked.
- Be concise — credits are deducted per reply.`;

  // ── 4. Icon Library ─────────────────────────────────────────────────────────
  const iconSection = showIcons
    ? `\
## ICONS — "Image" values only, never write in prose
${chunk(ICONS, 4).map(row => row.join(" | ")).join("\n")}
Usage:  "Image":"rbxassetid://<ID>"
Groups: Headers→Star/Crown/Stats/Trophy | Shop→Cart/Coin/Cash/Diamond
        Social→Player/Friend/Chat       | System→Settings/Info/Warning/Checkmark/Close
        Combat→Sword/Shield/Axe/Skull   | Inventory→Backpack/Box/Chest/Key`
  ...
  const soundSection = showSounds
    ? `\
## SOUNDS — "SoundId" values only, never write in prose
${chunk(SOUNDS, 3).map(row => row.join(" | ")).join("\n")}
Usage:    "SoundId":"rbxassetid://<ID>"
Defaults: UI→Vol 0.5 / Looped false / parent SoundService
          Combat→Vol 0.8 / Looped false / parent Part
...
DEFAULT PARENTS:
  RemoteEvent/Function → ReplicatedStorage  |  Script → ServerScriptService
  LocalScript → StarterPlayerScripts        |  ModuleScript → ReplicatedStorage
  ScreenGui/BillboardGui/SurfaceGui → StarterGui  |  Part/Model → Workspace

PROPERTY COERCION:
  Color3  : {r,g,b} | "r,g,b" | "#RRGGBB"
  Vector3 : {x,y,z} | "x,y,z"
...
ACTIONS:
  ...
  set_properties(name, property?, value?, properties?)
    shortcuts: gravity walk_speed jump_power clock_time brightness
               fog_end fog_start streaming_enabled respawn_time
  ...
  ${ptEnabled
    ...
    Run AFTER all create/inject actions. Auto-cleans injected scripts.`
    : "play_test → DISABLED."}
...
RUN_CODE MODES:
  pipeline    : steps:[{op, target?, name?, class?, parent?, property?, value?, properties?}]
                op = set | create | delete | clone | parent | rename | anchor | unanchor | call
  expression  : "Service.Prop.SubProp"  (read-only eval)
