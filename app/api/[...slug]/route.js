...
// v4: + lib/app/chats & lib/app/system-prompt
...
// ── lib/app (NEW) ─────────────────────────────────────────────────────────────
import chatsHandler     from '../../../lib/app/chats.js';
import sysPromptHandler from '../../../lib/app/system_prompt.js';
...
const ROUTES = {
  ...
  'chats':           chatsHandler,
  // GET | POST           →  /api/system-prompt
  'system-prompt':   sysPromptHandler,
};
...
// ─── ADAPTER: NextRequest → req/res (Pages Router style) ─────────────────────
...
async function handle(request, context) {
  ...
  try {
    ...
    const endpoint = (slug[0] ?? '')
      .toLowerCase()
      .replace(/[^a-z0-9\-]/g, '') // allow hyphens for 'google-callback', 'system-prompt'
      || '';
