...
export default function DashboardPage() {
  ...
  const [userData, setUserData]         = useState<UserData>({ projects: [] });
  ...
  const [saveStatus, setSaveStatusState] = useState<{ state: string; msg: string } | null>(null);
  const [syncBar, setSyncBarState]      = useState('');
  ...
  const [dailyStatus, setDailyStatus]   = useState<{ label: string; canClaim: boolean }>({ label: '', canClaim: false });

  const sessionRef  = useRef<Session | null>(null);
  const userDataRef = useRef<UserData>({ projects: [] });
  const queueRef    = useRef<PendingItem[]>([]);
  const toastTimer  = useRef<ReturnType<typeof setTimeout> | null>(null);
  const saveTimer   = useRef<ReturnType<typeof setTimeout> | null>(null);

  /* ── helpers ── */
  function showToast(msg: string, color: string = 'var(--cyan)', dur = 2800) {
    if (toastTimer.current) clearTimeout(toastTimer.current);
    setToastState({ msg, color });
    toastTimer.current = setTimeout(() => setToastState(null), dur);
  }

  function setSaveStatus(state: string, msg: string) {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    setSaveStatusState({ state, msg });
    if (state === 'saved') saveTimer.current = setTimeout(() => setSaveStatusState(null), 3000);
    if (state === 'error') saveTimer.current = setTimeout(() => setSaveStatusState(null), 6000);
  }

  function saveLocal(s: Session) {
    try { localStorage.setItem('nexus_session', JSON.stringify(s)); } catch { /* noop */ }
  }

  function savePendingQueue(q: PendingItem[]) {
    try { localStorage.setItem('nexus_pending_queue', JSON.stringify(q)); } catch { /* noop */ }
    queueRef.current = q;
    setPendingQueue([...q]);
  }

  function getLimit(ud: UserData): number {
  ...
  function getCreditsDisplay(ud: UserData): string {
    const c = ud.credits ?? 30;
    const plan = (ud.plan || 'free').toLowerCase();
    const roles = ud.roles || [];
    const unlimited = plan === 'owner' || plan === 'unlimited' || roles.includes('owner');
    return unlimited ? '∞' : parseFloat(String(c)).toFixed(0);
  }

  function getPlanLabel(ud: UserData): { label: string; cls: string } {
    const plan  = (ud.plan || 'free').toLowerCase();
    const roles = ud.roles || [];
  ...
  function computeDailyStatus(ud: UserData) {
    const plan  = (ud.plan || 'free').toLowerCase();
    const roles = ud.roles || [];
    if (plan === 'owner' || roles.includes('owner') || roles.includes('admin')) {
      setDailyStatus({ label: 'Credits Unlimited', canClaim: false });
      return;
    }
    ...
    if (last) {
      ...
      if (diff < 24) {
        ...
        setDailyStatus({ label: `Daily (${h}h remaining)`, canClaim: false });
        return;
      }
    }
    setDailyStatus({ label: 'Claim Daily Credits', canClaim: true });
  }

  /* ── save to server ── */
  const _attemptSave = useCallback(async (payload: Record<string, unknown>, maxRetry: number, showStatus?: boolean): Promise<boolean> => {
    ...
    for (let attempt = 1; attempt <= maxRetry; attempt++) {
      try {
        ...
        if (result.success || result.data) {
          if (result.data) {
            const newUd = { ...userDataRef.current };
            ...
            setUserData(newUd);
            setProjects(newUd.projects || []);
            if (sessionRef.current) {
              const newSession = { ...sessionRef.current, data: { ...sessionRef.current.data, ...result.data } };
              sessionRef.current = newSession;
              setSession(newSession);
              saveLocal(newSession);
            }
          }
          if (showStatus) setSaveStatus('saved', 'Saved to server');
          return true;
        }
        throw new Error(result.error || 'Server rejected');
      } catch (e: unknown) {
        const isAbort = (e as Error).name === 'AbortError';
        console.warn(`[NEXUS] save attempt ${attempt}/${maxRetry}:`, isAbort ? 'timeout' : (e as Error).message);
        if (attempt < maxRetry) {
          if (showStatus) setSaveStatus('retry', `Retrying... (${attempt}/${maxRetry})`);
          await sleep(delay);
  ...
  const saveToServer = useCallback(async (opts: { showStatus?: boolean } = {}): Promise<boolean> => {
    const ud   = userDataRef.current;
    const sess = sessionRef.current;
    if (!sess || !ud) return false;
    const username = (sess.user.username || '').toLowerCase();
    if (opts.showStatus) setSaveStatus('saving', 'Saving to server...');
    setSyncBarState('syncing');
    const safeData = {
    ...
    const payload = { user: username, robloxId: sess.user.robloxId || '', data: safeData };
    const ok = await _attemptSave(payload, RETRY_MAX, opts.showStatus);
    if (ok) {
      setSyncBarState('ok');
      setTimeout(() => setSyncBarState(''), 1500);
    } else {
      setSyncBarState('error');
      setTimeout(() => setSyncBarState(''), 3000);
      const newQ = [...queueRef.current, { type: 'save', payload, ts: Date.now() }];
      savePendingQueue(newQ);
      if (opts.showStatus) setSaveStatus('error', 'Save failed — queued for retry');
    }
    return ok;
  }, [_attemptSave]);
  ...
  const retryPendingQueue = useCallback(async () => {
    ...
    savePendingQueue([]);
    let failed: PendingItem[] = [];
    for (const item of queue) {
      if (item.type === 'save') {
        const ok = await _attemptSave(item.payload as Record<string, unknown>, 1);
        if (!ok) failed.push(item);
      }
    }
    if (failed.length > 0) {
      savePendingQueue([...queueRef.current, ...failed]);
    } else if (queue.length > 0) {
      showToast('All changes synced to server', 'var(--green)');
    }
  }, [isOnline, _attemptSave]);
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        if (Array.isArray(q)) { queueRef.current = q; setPendingQueue(q); }
      } catch { /* noop */ }
      ...
      try {
        ...
        clearTimeout(tId);
        if (r.ok) {
          const d: UserData = await r.json();
          if (d && typeof d === 'object') {
            if (!Array.isArray(d.projects)) d.projects = [];
            userDataRef.current = d;
            setUserData(d);
            setProjects(d.projects || []);
            computeDailyStatus(d);
            const newSession = { ...sess, data: { ...sess.data, ...d } };
            sessionRef.current = newSession;
            setSession(newSession);
            saveLocal(newSession);
          }
        ...
        const fallback: UserData = sess.data || {};
        if (!Array.isArray(fallback.projects)) fallback.projects = [];
        userDataRef.current = fallback;
        setUserData(fallback);
        setProjects(fallback.projects || []);
        computeDailyStatus(fallback);
      }
      ...
      // Online listeners
      const handleOnline  = () => { setIsOnline(true); setTimeout(retryPendingQueue, 1000); };
      const handleOffline = () => setIsOnline(false);
      window.addEventListener('online', handleOnline);
      window.addEventListener('offline', handleOffline);

      if (navigator.onLine && queueRef.current.length > 0) {
        setTimeout(retryPendingQueue, 2000);
      }
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key === 'Escape') {
        setDeleteModal(null); setLogoutModal(false); setSettingsModal(false); setDdOpen(false);
      }
  ...
  async function createProject() {
    ...
    if (!name) {
      ...
      showToast('Please enter a project name', 'var(--yellow)');
      return;
    }
    const limit = getLimit(userDataRef.current);
    if ((userDataRef.current.projects || []).length >= limit) {
      showToast('Project limit reached — upgrade to Pro', 'var(--pink)');
      return;
    }
    setCreating(true);
    const pid  = generateProjectId();
    const proj: Project = { id: pid, name, createdAt: new Date().toISOString() };
    const newUd = { ...userDataRef.current, projects: [proj, ...(userDataRef.current.projects || [])] };
    userDataRef.current = newUd;
    setUserData(newUd);
    setProjects(newUd.projects || []);
    setProjectName('');
    if (sessionRef.current) {
      const newSession = { ...sessionRef.current, data: { ...sessionRef.current.data, ...newUd } };
      sessionRef.current = newSession;
      saveLocal(newSession);
    }
    const saved = await saveToServer({ showStatus: true });
    setCreating(false);
    if (saved) showToast('Project created — opening chat...', 'var(--green)', 1800);
    else showToast('Saved locally — will sync when online', 'var(--orange)', 2500);
    await notifyPlugin(pid, name);
  ...
  async function executeDelete(id: string) {
    setDeleteModal(null);
    const newUd = { ...userDataRef.current, projects: (userDataRef.current.projects || []).filter(p => p.id !== id) };
    userDataRef.current = newUd;
    setUserData(newUd);
    setProjects(newUd.projects || []);
    const ok = await saveToServer({});
    showToast(ok ? 'Project deleted' : 'Project deleted locally — will sync when online', ok ? 'var(--dim2)' : 'var(--orange)');
  }
  ...
  async function claimDaily() {
    ...
    const newUd = { ...ud, credits: (parseFloat(String(ud.credits)) || 0) + n, lastClaim: new Date().toISOString() };
    userDataRef.current = newUd;
    setUserData(newUd);
    computeDailyStatus(newUd);
    await saveToServer({});
  ...
  async function handleRedeem() {
    ...
    try {
      const r = await fetch('/api/redeem', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: redeemCode.trim().toUpperCase(), user: (sess.user.username || '').toLowerCase(), userId: sess.user.robloxId || '' })
      });
      const d = await r.json();
      if (d.success) {
        const newUd = { ...userDataRef.current, credits: (parseFloat(String(userDataRef.current.credits)) || 0) + parseFloat(d.credits || 0) };
        userDataRef.current = newUd;
        setUserData(newUd);
        await saveToServer({});
        setRedeemMsg({ text: `+${d.credits} CR redeemed successfully!`, ok: true });
        setRedeemCode('');
      } else {
        setRedeemMsg({ text: 'Error: ' + (d.error || 'Invalid code'), ok: false });
      }
    } catch {
      setRedeemMsg({ text: 'Failed to connect to server', ok: false });
    }
  }

  /* ── derived values ── */
  const creditsDisplay = getCreditsDisplay(userData);
  const { label: planLabel, cls: planCls } = getPlanLabel(userData);
  ...
  const filteredProjects = (() => {
    let arr = searchQuery
      ...
      : [...allProjects];
    if (sortOrder === 'newest') arr.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    else if (sortOrder === 'oldest') arr.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    else if (sortOrder === 'name') arr.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    return arr;
  })();

  const pendingIds = new Set(pendingQueue.flatMap(q =>
    ((q.payload?.data as UserData)?.projects || []).map(p => p.id)
  ));

  const av = session?.user?.avatar || '/nexusai.png';
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        #__nx_loader{position:fixed;inset:0;background:var(--bg);z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;transition:opacity .45s ease;}
        #__nx_loader.hide{opacity:0;pointer-events:none;}
        ...
        .loader-bar{height:100%;background:linear-gradient(90deg,var(--cyan),var(--purple));border-radius:2px;transition:width .3s ease;}
        /* NAV */
        .nav{position:sticky;top:0;z-index:200;display:flex;align-items:center;justify-content:space-between;padding:0 32px;height:58px;background:rgba(3,3,18,.95);border-bottom:1px solid var(--b);backdrop-filter:blur(28px);}
        .nav-logo{font-family:"Orbitron",sans-serif;font-size:14px;font-weight:900;background:linear-gradient(135deg,var(--cyan),var(--purple));-webkit-background-clip:text;-webkit-text-fill-color:transparent;text-decoration:none;letter-spacing:2.5px;display:flex;align-items:center;gap:10px;cursor:pointer;}
        .nav-logo-icon{width:30px;height:30px;border-radius:8px;overflow:hidden;border:1px solid var(--b);flex-shrink:0;box-shadow:0 0 12px rgba(0,229,255,.15);}
        .nav-logo-icon img{width:100%;height:100%;object-fit:cover;display:block;}
        .nav-right{display:flex;align-items:center;gap:10px;}
        .nav-credits{display:flex;align-items:center;gap:6px;padding:6px 14px;border-radius:22px;background:rgba(255,214,0,.05);border:1px solid rgba(255,214,0,.2);font-size:11px;color:var(--yellow);font-weight:600;cursor:pointer;transition:.2s;text-decoration:none;}
        .nav-credits:hover{border-color:rgba(255,214,0,.45);background:rgba(255,214,0,.1);transform:translateY(-1px);}
        .nav-credits svg{width:12px;height:12px;stroke:currentColor;fill:none;stroke-width:2.2;}
        /* USER PILL */
        ...
        .user-pill:hover,.user-pill.open{border-color:var(--bb);}
        .user-av{width:30px;height:30px;border-radius:50%;border:1.5px solid rgba(0,229,255,.35);object-fit:cover;background:var(--bg3);flex-shrink:0;}
        .user-name{font-size:11px;color:var(--text);font-weight:500;}
        ...
        /* DROPDOWN */
        .user-dd{position:absolute;top:calc(100% + 8px);right:0;width:256px;background:var(--bg2);border:1px solid var(--b);border-radius:14px;box-shadow:0 24px 64px rgba(0,0,0,.95);z-index:9999;overflow:hidden;animation:fadeIn .16s ease;}
        ...
        /* MAIN */
        .main{max-width:1080px;margin:0 auto;padding:44px 24px 80px;position:relative;z-index:1;}
        /* PAGE HEADER */
        ...
        .stat-card{background:var(--bg2);border:1px solid var(--b);border-radius:var(--r);padding:20px;display:flex;align-items:center;gap:14px;transition:.22s;cursor:default;}
        ...
        .project-input:focus{border-color:rgba(0,229,255,.4);box-shadow:0 0 0 3px rgba(0,229,255,.06);}
        .project-input.error{border-color:rgba(255,45,107,.6);box-shadow:0 0 0 3px rgba(255,45,107,.08);}
        ...
        .btn-spinner{width:14px;height:14px;border:2px solid rgba(3,3,18,.2);border-top-color:#030312;border-radius:50%;animation:spin .7s linear infinite;}
        /* SAVE STATUS */
        .save-status{margin-top:10px;font-size:10px;display:flex;align-items:center;gap:6px;}
        ...
        .save-status.retry{color:var(--orange);}
        /* NOTICE */
        ...
        /* QUEUE NOTICE */
        .queue-notice{display:flex;align-items:center;gap:10px;background:rgba(255,140,0,.04);border:1px solid rgba(255,140,0,.18);border-radius:10px;padding:11px 15px;margin-top:12px;}
        ...
        .queue-notice button{padding:3px 10px;border-radius:5px;border:1px solid rgba(255,140,0,.3);background:rgba(255,140,0,.08);color:var(--orange);font-size:9px;cursor:pointer;font-family:"JetBrains Mono",monospace;}
        /* SEARCH */
        ...
        .search-input:focus{border-color:rgba(0,229,255,.3);}
        .search-input::placeholder{color:var(--dim);}
        .sort-select{background:var(--bg2);border:1px solid var(--b);border-radius:9px;padding:9px 12px;color:var(--text);font-family:"JetBrains Mono",monospace;font-size:10px;outline:none;cursor:pointer;}
        /* SECTION HEADER */
        ...
        .project-card:hover{transform:translateY(-3px);border-color:rgba(0,229,255,.28);box-shadow:0 12px 36px rgba(0,0,0,.4);}
        ...
        /* OVERLAY */
        .overlay{position:fixed;inset:0;background:rgba(3,3,18,.88);z-index:500;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(8px);}
        .modal-box{background:var(--bg2);border:1px solid var(--b);border-radius:16px;padding:28px;width:420px;max-width:94vw;box-shadow:0 32px 80px rgba(0,0,0,.95);animation:fadeIn .2s ease;}
        ...
        .modal-desc .highlight{color:var(--cyan);font-weight:600;}
        ...
        .settings-row{display:flex;align-items:center;justify-content:space-between;padding:8px 0;font-size:11px;gap:10px;}
        ...
        .redeem-input{flex:1;background:var(--bg3);border:1px solid var(--b);border-radius:7px;padding:8px 12px;color:white;font-family:"JetBrains Mono",monospace;font-size:11px;outline:none;transition:.18s;}
        .redeem-input:focus{border-color:rgba(0,229,255,.35);}
        /* OFFLINE BANNER */
        .offline-banner{position:fixed;top:0;left:0;right:0;z-index:9998;background:rgba(255,140,0,.12);border-bottom:1px solid rgba(255,140,0,.3);padding:7px 20px;display:flex;align-items:center;justify-content:center;gap:8px;font-size:10px;color:var(--orange);}
        .offline-banner svg{width:13px;height:13px;stroke:currentColor;fill:none;stroke-width:2;flex-shrink:0;}
        .offline-retry{padding:3px 10px;border-radius:5px;border:1px solid rgba(255,140,0,.4);background:rgba(255,140,0,.1);color:var(--orange);font-size:9px;cursor:pointer;font-family:"JetBrains Mono",monospace;margin-left:8px;}
        /* SYNC BAR */
        .sync-bar{position:fixed;bottom:0;left:0;right:0;z-index:300;height:2px;}
        .sync-bar.syncing{background:linear-gradient(90deg,transparent,var(--cyan),var(--purple),transparent);background-size:200% 100%;animation:shimmer 1.5s linear infinite;}
        .sync-bar.error{background:var(--pink);}
      ...
      {/* LOADER */}
      {loaderVisible && (
        <div id="__nx_loader" className={loaderProgress === 100 ? 'hide' : ''}>
          <div className="loader-logo">NEXUS AI</div>
          <div className="loader-ring" />
          <div className="loader-progress"><div className="loader-bar" style={{ width: loaderProgress + '%' }} /></div>
      ...
      {!isOnline && (
        <div className="offline-banner">
          ...
          <button className="offline-retry" onClick={retryPendingQueue}>Retry Now</button>
        </div>
      )}

      {/* SYNC BAR */}
      <div className={`sync-bar${syncBar ? ' ' + syncBar : ''}`} />

      {/* NAV */}
      <nav className="nav">
        <div className="nav-logo" onClick={() => window.location.href = '/dashboard'}>
          <div className="nav-logo-icon">
            <img src="/nexusai.png" alt="" onError={e => { (e.target as HTMLImageElement).style.display='none'; }} />
          </div>
          NEXUS AI
        </div>
        <div className="nav-right">
          <a href="/payment" className="nav-credits">
            <svg viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
            <span>{creditsDisplay}</span> CR
          </a>
          <div className="user-pill-wrap">
            <div className={`user-pill${ddOpen ? ' open' : ''}`} onClick={() => setDdOpen(v => !v)}>
              <img className="user-av" src={av} alt="" onError={e => { (e.target as HTMLImageElement).src='/nexusai.png'; }} />
            ...
            {ddOpen && (
              <div className="user-dd">
                <div className="ud-hdr">
                  <img className="ud-av" src={av} alt="" onError={e => { (e.target as HTMLImageElement).src='/nexusai.png'; }} />
                  <div>
                    <div className="ud-name">@{session?.user?.username || 'username'}</div>
                    <div className="ud-role">{planLabel} Plan</div>
                  </div>
                </div>
                <div className="ud-section">
                  <div className="ud-item" onClick={() => { setSettingsModal(true); setDdOpen(false); }}>
                ...
                <div className="ud-section">
                  <div className="ud-item" onClick={() => { claimDaily(); setDdOpen(false); }}>
                    ...
                    {dailyStatus.label || 'Claim Daily Credits'}
                  </div>
                </div>
                <div className="ud-divider"/>
                <div className="ud-section">
      ...
      <div className="main">
        ...
        <div className="page-header">
          <div className="header-left">
            <div className="header-av-wrap">
              <img src={av} alt="" onError={e => { (e.target as HTMLImageElement).src='/nexusai.png'; }} />
            </div>
        ...
        <div className="create-card">
          ...
          <div className="input-row">
            ...
            />
            <button className="btn-create" onClick={createProject} disabled={creating || atLimit}>
              {creating
                ? <><div className="btn-spinner"/></>
                : <><span>CREATE</span><svg viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg></>
              }
            </button>
          </div>
          {saveStatus && (
          ...
          {pendingQueue.length > 0 && (
            <div className="queue-notice">
              ...
              <p><span>{pendingQueue.length}</span> unsaved change(s) queued — will sync automatically when online.</p>
              <button onClick={retryPendingQueue}>Retry</button>
            </div>
          )}
        ...
        <div className="section-header">
          ...
          <div className="section-count">{filteredProjects.length} project{filteredProjects.length !== 1 ? 's' : ''}</div>
        </div>
        ...
        <div className="projects-grid">
          {loading ? (
            <div className="empty-state">
              <div className="empty-icon"><svg viewBox="0 0 24 24" style={{ animation: 'spin 1s linear infinite' }}><circle cx="12" cy="12" r="10"/></svg></div>
            ...
            <div className="empty-state">
              ...
              {searchQuery ? (
                <>
                  ...
                  <p>No projects match &ldquo;<strong>{esc(searchQuery)}</strong>&rdquo;.<br/>Try a different search term.</p>
                </>
              ) : (
                <>
            ...
            filteredProjects.map((p, i) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`project-card${isPending ? ' pending-save' : ''}`}
                ...
                >
                  ...
                  <div className="project-meta">
                    ...
                    <span className={`meta-status${isPending ? ' pending' : ''}`}>
                      <span className="status-dot"/>
                      {isPending ? 'Pending sync' : 'Active'}
                    </span>
                  </div>
      ...
      {/* DELETE MODAL */}
      {deleteModal && (
        <div className="overlay" onClick={() => setDeleteModal(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div className="modal-icon">
              <svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
            </div>
            <div className="modal-title">Delete Project?</div>
            <div className="modal-desc">Are you sure you want to delete <span className="highlight">&ldquo;{deleteModal.name}&rdquo;</span>? All chat history will be permanently removed. This cannot be undone.</div>
            <div className="modal-btns">
              <button className="modal-btn cancel" onClick={() => setDeleteModal(null)}>Cancel</button>
              <button className="modal-btn danger" onClick={() => executeDelete(deleteModal.id)}>Delete Project</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* LOGOUT MODAL */}
      {logoutModal && (
        <div className="overlay" onClick={() => setLogoutModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div className="modal-icon">
              <svg viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
            </div>
            ...
            <div className="modal-btns">
              ...
              <button className="modal-btn danger" onClick={() => { localStorage.removeItem('nexus_session'); window.location.replace('/'); }}>Sign Out</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="overlay" onClick={() => setSettingsModal(false)}>
          <div className="modal-box wide" onClick={e => e.stopPropagation()}>
            <div className="settings-hdr">
              ...
              <button className="settings-close" onClick={() => setSettingsModal(false)}>
            ...
            <div className="settings-sec">
              ...
              <div className="settings-av-row">
                <img className="settings-av" src={av} alt="" onError={e => { (e.target as HTMLImageElement).src='/nexusai.png'; }} />
                <div>
                  <div className="settings-av-name">@{session?.user?.username || 'username'}</div>
            ...
            <div className="settings-sec">
              ...
              <div className="settings-row">
                <span style={{ fontSize: 10, color: 'var(--dim2)', flex: 1 }}>{dailyStatus.label}</span>
                <button className="settings-btn success" onClick={claimDaily} disabled={!dailyStatus.canClaim}>Claim Daily</button>
              </div>
            </div>
            {/* Redeem Code */}
            <div className="settings-sec">
              ...
              <div style={{ fontSize: 10, color: 'var(--dim2)', marginBottom: 10 }}>
                Get codes on <a href="https://discord.gg/FzAF48mvK5" target="_blank" rel="noreferrer" style={{ color: 'var(--cyan)', textDecoration: 'none' }}>NEXUS STUDIO Discord</a>
              </div>
              ...
              {redeemMsg && (
                <div style={{ fontSize: 10, marginTop: 8, color: redeemMsg.ok ? 'var(--green)' : 'var(--pink)' }}>{redeemMsg.text}</div>
              )}
            </div>
            {/* Studio Plugin */}
            <div className="settings-sec">
            ...
            <div className="settings-sec">
              ...
              <div className="settings-row">
                ...
                <button className="settings-btn danger" onClick={() => { setSettingsModal(false); setLogoutModal(true); }}>Sign Out</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* TOAST */}
      {toast && (
        <div className="nx-toast" style={{ color: toast.color }}>
          <svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
          <span>{toast.msg}</span>
        </div>
      )}
      ...
      {/* CLICK OUTSIDE DD */}
      {ddOpen && <div style={{ position: 'fixed', inset: 0, zIndex: 199 }} onClick={() => setDdOpen(false)} />}
    </>
  );
}