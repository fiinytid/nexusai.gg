// api/control.js — NEXUS AI Control API  ·  FIXED v13.1
// ════════════════════════════════════════════════════════
// FIXES v13.1:
//  [1] /api/version route — 404 karena tidak ada handler path terpisah.
//      Sekarang GET ?version=1 DAN path /api/version keduanya return data versi.
//  [2] checkVersion 404 — diperbaiki dengan fix [1] + Security whitelist sudah cover.
//  [3] nxai_connect alias — ditambahkan ke pluginCallbacks (sebelumnya cuma di komentar).
//  [4] san() null-safe — sekarang handle null/undefined tanpa crash.
//  [5] verifyAdminToken timing-safe — padding diperbaiki agar tidak overflow buffer.
//  [6] getSession lastSeen — tidak update sebelum TTL expiry (race condition diperbaiki).
//  [7] CORS headers — Roblox HttpService tidak kirim Origin, jadi set * hanya jika
//      benar-benar tidak ada origin. Tambah 'x-nexus-nonce' ke allow-headers.
//  [8] sendError — pastikan semua path mengembalikan HTTP status code yang tepat.
//  [9] Queue age filter — ditambahkan juga saat push agar queue tidak penuh stale data.
// [10] robustJsonParse — isolasi scope setiap attempt agar tidak ada variable bleed.
// [11] set_properties & set_property callback — ditambahkan sebagai plugin callback.
// [12] disconnect callback — ditambahkan agar plugin bisa sign-off session.
// [13] get_webhook GET handler — ditambahkan.
// [14] Missing 'project' field di poll response — sekarang include projectId & name.
// ════════════════════════════════════════════════════════════════════════════
...
export const WEB_VERSION             = 'V13.1';
...
// FIX [10]: Setiap attempt diisolasi dalam closure agar tidak ada variable bleed
function robustJsonParse(raw) {
  ...
  // Attempt 3: trailing commas + quote unquoted keys
  try {
    return JSON.parse(
      s.replace(/,\s*([}\]])/g, '$1')
  ...
  // Attempt 4: Lua-style nil→null and = vs :
  try {
    return JSON.parse(
      s.replace(/,\s*([}\]])/g, '$1')
...
// ────────────────────────────────────────────────────────────────────────────
//  Brace-depth JSON object finder (tidak bergantung pada regex [^`]*)
// ────────────────────────────────────────────────────────────────────────────
...
// FIX [6]: update lastSeen SEBELUM TTL check agar tidak ada stale reads
...
// FIX [5]: padding diperbaiki — gunakan panjang maksimum dari kedua string
function verifyAdminToken(req) {
  ...
  try {
    // FIX: gunakan panjang yang sama (max dari keduanya) untuk timing-safe compare
...
// FIX [9]: Filter stale commands saat PUSH juga, bukan hanya saat drain
...
// ════════════════════════════════════════════════════════════════════════════
//  SECURITY HEADERS
// FIX [7]: CORS yang benar — Roblox HttpService tidak kirim Origin header
// ════════════════════════════════════════════════════════════════════════════
...
function setSecurityHeaders(req, res) {
  ...
  if (origin) {
    // Request dari browser — strict origin whitelist
    res.setHeader('Access-Control-Allow-Origin',
      ALLOWED_ORIGINS.has(origin) ? origin : 'null');
    res.setHeader('Vary', 'Origin');
  } else {
    // Request dari Roblox plugin (tidak ada Origin header) — izinkan
    res.setHeader('Access-Control-Allow-Origin', '*');
  }
...
// FIX [8]: sendError memastikan HTTP status code benar, tidak selalu 200
// ════════════════════════════════════════════════════════════════════════════
...
export default async function handler(req, res) {
  try {
    ...
    console.error('[NEXUS v13.1] Unhandled error:', err?.message || err);
    try {
...
async function handleGet(req, res) {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // FIX [1] & [2]: /api/version — plugin memanggil endpoint ini untuk
  // checkVersion. Sebelumnya hanya ?version=1 yang di-handle, menyebabkan
  // HTTP 404 karena path /api/version tidak dikenali.
  // Sekarang KEDUANYA (?version=1 dan tanpa parameter di path /api/version)
  // mengembalikan data versi yang sama.
  // ─────────────────────────────────────────────────────────────────────────
  if (q.version === '1' || Object.keys(q).length === 0) {
    return res.status(200).json({
      ...
      changelog:               `v13.1 — Fixed /api/version 404, nxai_connect, san() null-safe, timing-safe tokens, CORS headers, queue stale-filter, robustJsonParse isolation, set_properties callback, disconnect callback.`,
      security_model: {
  ...
  // FIX [13]: get_webhook GET handler
  ...
  // FIX [14]: include semua project fields di poll response
  return res.status(200).json({
    ok:    true,
    queue,
    count:         queue.length,
...
async function handlePost(req, res) {
  ...
  const pluginCallbacks = {
    ...
    // FIX [3]: nxai_connect — plugin handshake alias (sebelumnya hanya di komentar)
    nxai_connect: () => {
    ...
    // FIX [12]: disconnect callback agar plugin bisa sign-off session dengan bersih
    nxai_disconnect: () => {
      const sess = sessionStore.get(san(resolvedUser));
      if (sess) {
        appendSessionAudit(san(resolvedUser), 'disconnect', {});
        sessionStore.delete(san(resolvedUser));
      }
    ...
    // FIX [11]: set_properties / set_property callback yang sebelumnya tidak ada
