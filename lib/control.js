// api/control.js — NEXUS AI (SECURE v12 - COMPLETE PRODUCTION UPGRADE)
// ════════════════════════════════════════════════════════════════════════════
// v12 UPGRADE HIGHLIGHTS (dari v10):
//   1. PRIORITY QUEUE — critical/high/normal/low priority levels
//   2. COMMAND DEDUPLICATION — cegah command duplikat dalam queue
//   3. IN-MEMORY API CACHE — TTL cache untuk Roblox API calls (reduce latency)
//   4. IP RATE LIMITING — rate limit berbasis IP + user (dual-layer)
//   5. ACTION ALIASES — 50+ alias agar backward-compatible
//   6. HMAC SIGNATURE VERIFY — opsional signing untuk plugin requests
//   7. MULTI-TARGET SUPPORT — kirim 1 command ke banyak user sekaligus
//   8. PRIORITY COMMAND — command dengan priority tinggi langsung dieksekusi
//   9. WEBHOOK SUPPORT — POST ke URL eksternal saat command selesai
//  10. GAME INFO API — ambil info Roblox game dari UniverseId/PlaceId
//  11. AVATAR API — ambil info Roblox avatar user
//  12. QUEUE STATS — statistik queue per user
//  13. ACTION CATEGORIES — organize 200+ actions ke dalam kategori
//  14. BATCH EXPAND SERVER-SIDE — expand semua batch sebelum push ke queue
//  15. SESSION AUDIT LOG — log semua session events
//  16. COMMAND HISTORY PER USER — riwayat command per user
//  17. PLUGIN HEARTBEAT MONITOR — deteksi koneksi putus lebih akurat
//  18. AUTO QUEUE CLEANUP — hapus command > 30 menit dari queue
//  19. EXPANDED DOCS INDEX — 60+ entri dokumentasi Luau & Roblox
//  20. STRUCTURED HEALTH ENDPOINT — metrics lengkap untuk monitoring
// ════════════════════════════════════════════════════════════════════════════

import {
  readFileSync, writeFileSync, existsSync,
  unlinkSync, readdirSync, statSync, mkdirSync,
} from 'fs';
...
const TMP                   = '/tmp';
const SESSION_TTL           = 24 * 60 * 60 * 1_000;      // 24 jam
const MAX_QUEUE_SIZE        = 300;                         // ↑ dari 200
const MAX_PRIORITY_QUEUE    = 50;                          // priority queue per user
const MAX_LOG_ENTRIES       = 500;                         // ↑ dari 300
const MAX_HIST_ENTRIES      = 200;                         // ↑ dari 150
const MAX_LOGSVC_ENTRIES    = 1_000;                       // ↑ dari 500
const MAX_MENTION_ENTRIES   = 100;                         // ↑ dari 50
const MAX_USER_CMD_HIST     = 100;                         // riwayat command per user

// Rate Limiting
const RATE_LIMIT_PER_MIN    = 150;                         // ↑ dari 120
const RATE_LIMIT_IP_PER_MIN = 300;                         // IP-based limit
const RATE_LIMIT_BURST      = 20;                          // max burst dalam 5 detik
...
const COMMAND_DEDUP_WINDOW  = 500;                         // ms - jangan push command identik

// Cache TTL (ms)
const CACHE_TOOLBOX_TTL     = 5 * 60_000;                  // 5 menit
const CACHE_ASSET_TTL       = 30 * 60_000;                 // 30 menit
const CACHE_USERINFO_TTL    = 10 * 60_000;                 // 10 menit
const CACHE_GAMEINFO_TTL    = 15 * 60_000;                 // 15 menit

// Queue command ages
const QUEUE_CMD_MAX_AGE     = 30 * 60_000;                 // hapus command > 30 menit
...
const priorityQFile    = u => f('nqp',      u);   // NEW: priority queue
...
const userCmdHistFile  = u => f('nucmdh',   u);   // NEW: per-user command history
const sessionAuditFile = u => f('nsessaud', u);   // NEW: session audit log

const LOG_FILE         = `${TMP}/nexus_log.json`;
const HIST_FILE        = `${TMP}/nexus_hist.json`;
const STATS_FILE       = `${TMP}/nexus_global_stats.json`;
const WEBHOOK_FILE     = u => f('nwebhook', u);   // NEW: webhook config per user
...
const dedupCache     = new Map();   // cmdHash → timestamp (prevent duplicate commands)

// ─── FILE PREFIXES (for cleanup) ─────────────────────────────────────────────
const FILE_PREFIXES = [
...
function sanPriority(val) {
  ...
  return valid.includes(val) ? val : 'normal';
}

// ════════════════════════════════════════════════════════════════════════════
// API CACHE (In-Memory dengan TTL)
// ════════════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════════════
// ROBUST JSON PARSER (dari v10, ditingkatkan)
// ════════════════════════════════════════════════════════════════════════════
...
function robustJsonParse(raw) {
  ...
  try { return JSON.parse(cleaned.replace(/,(\s*[}\]])/g, '$1')); } catch (_) {}
  try {
  ...
  try {
    return JSON.parse(cleaned
      ...
      .replace(/,(\s*[}\]])/g, '$1')
      .replace(/([{,\[]\s*)([a-zA-Z_][a-zA-Z0-9_]*)(\s*[=:](?![=:>]))/g, (_, p, k) => `${p}"${k}": `)
      .replace(/:\s*nil\b/g, ': null').replace(/:\s*true\b/g, ': true').replace(/:\s*false\b/g, ': false'));
  } catch (_) {}
  return null;
}
...
// ════════════════════════════════════════════════════════════════════════════
// ACTION ALIASES (NEW v12) — backward compatibility + shortcuts
// ════════════════════════════════════════════════════════════════════════════
const ACTION_ALIASES = {
  ...
  'run':               'run_lua',
  'exec':              'run_lua',
  'execute_text':      'execute_json',      // text alias
...
// ════════════════════════════════════════════════════════════════════════════
// VALID ACTIONS (expanded dari v10)
// ════════════════════════════════════════════════════════════════════════════
const VALID_ACTIONS = new Set([
  ...
  'delete_script', 'move_script', 'copy_script',
  'watch_script',                                         // NEW: watch script for changes
  ...
  'create_remote_property',                               // NEW

  // Workspace & Scan
  'scan_workspace', 'read_workspace', 'workspace_data', 'request_scan',
  'workspace_stats', 'workspace_tree',                    // NEW: get instance tree
  ...
  'list_services',
  'find_by_tag',                                          // NEW: find by CollectionService tag
  'find_by_attribute',                                    // NEW: find by attribute

  // Batch Operations
  'batch_commands', 'batch_modify', 'batch_create', 'batch_rename',
  'batch_set_property', 'batch_parent', 'batch_delete',   // NEW: batch_delete
  'batch_tag',                                            // NEW: batch add/remove tags
  'batch_attribute',                                      // NEW: batch set attributes
  ...
  'create_negative_part',                                 // NEW: CSG negative part
  ...
  'create_cframe_value',                                  // NEW
  'create_ray_value',                                     // NEW

  // Transform & Modify
  'modify_part', 'move_object', 'rotate_object', 'resize_object',
  'snap_to_grid', 'align_objects', 'randomize_colors',
  'mirror_object',                                        // NEW: mirror along axis
  'flip_object',                                          // NEW: flip X/Y/Z
  'center_object',                                        // NEW: center in workspace
  'fit_to_grid',                                          // NEW

  // Delete
  'delete_object', 'delete', 'remove',
  'delete_multiple', 'delete_children',
  'delete_empty_folders',                                 // NEW
  ...
  'select_object', 'select_multiple', 'deselect_all',     // NEW: deselect_all
  'set_property', 'set_value', 'copy_properties', 'rename_object',
  'lock_object', 'unlock_object',
  'toggle_visible', 'set_visible', 'set_enabled',
  'toggle_anchored',
  'set_attribute',                                        // NEW: set attribute value
  'get_attribute',                                        // NEW: get attribute value
  'set_tags',                                             // NEW: set multiple tags at once
  ...
  'create_local_player_gui',                              // NEW: create full LocalPlayerGui script
  ...
  'create_ui_flex_item', 'create_ui_scale',               // NEW

  // Welds & Joints
  'weld_parts', 'create_weld',
  'create_attachment', 'create_motor6d',
  'create_snap',                                          // NEW: Snap joint
  'create_glue',                                          // NEW: Glue joint
  ...
  'create_ragdoll',                                       // NEW: ragdoll rig
  'create_r6_rig',                                        // NEW: R6 rig template
  'create_r15_rig',                                       // NEW: R15 rig template
  ...
  'create_selection_box',                                 // NEW: SelectionBox highlight
  'create_selection_sphere',                              // NEW
  'create_box_handle_adornment',                          // NEW

  // Audio
  'create_sound', 'add_sound', 'create_sound_group',
  'play_sound',                                           // NEW: play_sound command
  'stop_sound',                                           // NEW

  // Decals & Textures
  'place_decal', 'place_texture',
  'create_special_decal',                                 // NEW
  ...
  'create_building',                                      // NEW: building template
  'create_road',                                          // NEW: road template
  'clear_workspace',
  ...
  'create_anticheat_script',                              // NEW
  'create_loading_screen',                                // NEW
  'create_mobile_controls',                               // NEW
  'create_camera_script',                                 // NEW
  'create_round_system',                                  // NEW: round/lobby system
  'create_inventory_system',                              // NEW
  'create_currency_system',                               // NEW: coins/gems system

  // External API (server-side)
  'search_toolbox',
  'search_docs',
  'get_game_info',                                        // NEW: get Roblox game info
  'get_avatar_info',                                      // NEW: get avatar info
  'get_user_info',                                        // NEW: direct user info
  'validate_asset',                                       // NEW: validate asset ID

  // Echo/Debug (NEW v12)
  'debug_info',                                           // NEW: return debug info
  'queue_status',                                         // NEW: return queue info
]);
...
const ADMIN_ONLY_ACTIONS = new Set([
  ...
  'delete_script', 'delete_empty_folders',
  'create_anticheat_script',
]);

// Internal plugin callback actions
...
// ════════════════════════════════════════════════════════════════════════════
// HMAC SIGNATURE VERIFICATION (NEW v12)
// ════════════════════════════════════════════════════════════════════════════

function verifyPluginHmac(req, body) {
  const secret = process.env.PLUGIN_HMAC_SECRET;
  if (!secret || secret.length < 16) return true;   // skip jika tidak dikonfigurasi
  const sig = req.headers?.['x-nexus-signature'] || req.headers?.['x-roblox-signature'] || '';
  if (!sig) return true;   // skip jika plugin belum mendukung signing
  try {
...
// ════════════════════════════════════════════════════════════════════════════
// RATE LIMITING (Enhanced — user + IP + burst)
// ════════════════════════════════════════════════════════════════════════════
...
function getClientIp(req) {
  return (req.headers?.['x-real-ip'] || req.headers?.['x-forwarded-for'] || '').toString().split(',')[0].trim();
}
...
// ════════════════════════════════════════════════════════════════════════════
// COMMAND DEDUPLICATION (NEW v12)
// ════════════════════════════════════════════════════════════════════════════

function isDuplicateCommand(cmd) {
  // Only dedup for some action types where duplicates are likely accidents
  ...
  const hash = crypto.createHash('md5')
    .update(JSON.stringify({ action: cmd.action, name: cmd.name, code: (cmd.code || '').substring(0, 100) }))
    .digest('hex');
...
function pushQueue(u, cmd, priority = 'normal') {
  ...
  // Strip stale commands from queue on every push
...
function drainQueue(u) {
  // Merge priority + normal queue, priority first
...
function isOnline(u)   { return (Date.now() - lastPoll(u)) < 8_000; }   // ↑ 7→8s tolerance
...
// Per-user command history (NEW v12)
...
function getUserCmdHistory(u, limit = 50) {
  return readJson(userCmdHistFile(u), []).slice(0, sanInt(limit, 50, 1, MAX_USER_CMD_HIST));
}
...
function getGlobalStats() {
  return readJson(STATS_FILE, {
    ...
    startedAt: Date.now(), userStats: {}, popularActions: {},
  });
}
...
// ════════════════════════════════════════════════════════════════════════════
// DATA HELPERS (same as v10, plus new webhook)
// ════════════════════════════════════════════════════════════════════════════
...
const saveMention        = (u, d) => { let l = readJson(mentionFile(u), []); l.unshift({ ...d, _ts: Date.now() }); if (l.length > MAX_MENTION_ENTRIES) l = l.slice(0, MAX_MENTION_ENTRIES); writeJson(mentionFile(u), l); };
...
// ════════════════════════════════════════════════════════════════════════════
// WEBHOOK DISPATCH (NEW v12) — fire & forget
// ════════════════════════════════════════════════════════════════════════════
...
async function robloxToolboxSearch(keyword, assetType = 'Model', limit = 10, cursor = null) {
  ...
  if (!apiKey) throw Object.assign(new Error('ROBLOX_OPEN_CLOUD_KEY tidak dikonfigurasi.'), { code: 503 });
  ...
  let resp;
  try {
    resp = await safeFetch(`https://apis.roblox.com/toolbox-service/v2/assets:search?${params}`, {
      method: 'GET',
      headers: { 'x-api-key': apiKey, 'Accept': 'application/json', 'User-Agent': `NexusAI/${WEB_VERSION}` },
    }, 12_000, 2);
  } catch (err) {
    throw Object.assign(new Error(`Gagal terhubung ke Roblox Toolbox API: ${err?.message || 'timeout'}`), { code: 502 });
  }

  if (resp.status === 401 || resp.status === 403)
    throw Object.assign(new Error('API Key tidak valid. Cek ROBLOX_OPEN_CLOUD_KEY.'), { code: resp.status });
  if (resp.status === 429)
    throw Object.assign(new Error('Rate limit Roblox Toolbox. Coba lagi.'), { code: 429 });
  if (!resp.ok) {
  ...
  let data;
  try { data = await resp.json(); } catch (_) { throw Object.assign(new Error('Response non-JSON dari Roblox Toolbox.'), { code: 502 }); }
...
async function validateAndPrepareAsset(assetId) {
  ...
  if (!id || id <= 0 || id > 99_999_999_999)
    throw Object.assign(new Error(`AssetId tidak valid: "${sanStr(String(assetId), 30)}"`), { code: 400 });
  ...
  if (!assetData) {
    try {
      const r = await safeFetch(`https://assetdelivery.roblox.com/v1/asset/?id=${id}`, { headers: { Accept: 'application/json' } }, 8_000, 1);
      if (r.ok || r.status === 302) assetData = { name: `Asset #${id}`, assetType: 'Model', creator: {} };
    } catch (_) {}
  }
  ...
  if (!assetData) {
    ...
    cacheSet(cacheKey, result, CACHE_ASSET_TTL / 4);   // shorter TTL for unverified
    return result;
  }
...
// ─── ROBLOX USER INFO (NEW v12, with cache) ───────────────────────────────────
...
async function searchLuauDocs(query, docType = 'all', limit = 5) {
  ...
  // Try Roblox Creator Docs API
  try {
    const params = new URLSearchParams({ query: q, type: docType === 'all' ? '' : docType, limit: String(maxRes), locale: 'en-us' });
    const resp = await safeFetch(`https://create.roblox.com/api/search/docs?${params}`, {
  ...
  const scored = localIndex
    .map(entry => {
      ...
      for (const t of tokens) {
        ...
        if (entry.title.toLowerCase() === t) score += 20;   // exact match bonus
      }
      return { ...entry, score };
    })
    .filter(e => e.score > 0)
  ...
  if (scored.length === 0) {
    return {
      results: [{ title: 'Dokumentasi Roblox Creator', url: 'https://create.roblox.com/docs', snippet: `Tidak ditemukan hasil untuk "${q}".`, category: 'fallback' }],
...
function buildLocalDocsIndex() {
  // Extended index dari v10 dengan 60+ entries
  return [
    { title: 'Instance', url: 'https://create.roblox.com/docs/reference/engine/classes/Instance', snippet: 'Kelas dasar semua objek. Method: FindFirstChild, WaitForChild, Destroy, Clone, GetChildren, GetDescendants, IsA.', category: 'api', keywords: 'instance object findfirstchild waitforchild destroy clone getchildren getdescendants parent name classname isa' },
    { title: 'Workspace', url: 'https://create.roblox.com/docs/reference/engine/classes/Workspace', snippet: 'Service utama tempat semua objek 3D. Gravity, CurrentCamera. game:GetService("Workspace") atau workspace.', category: 'api', keywords: 'workspace gravity camera service game world 3d' },
    { title: 'BasePart / Part / MeshPart', url: 'https://create.roblox.com/docs/reference/engine/classes/BasePart', snippet: 'Part fisik. Size, Position, CFrame, Anchored, CanCollide, BrickColor, Material, Transparency.', category: 'api', keywords: 'part basepart size position cframe anchored cancollide material transparency brickcolor meshpart union' },
    { title: 'CFrame', url: 'https://create.roblox.com/docs/reference/engine/datatypes/CFrame', snippet: 'Posisi + rotasi. CFrame.new(x,y,z), CFrame.Angles(rx,ry,rz), CFrame.lookAt(pos,target).', category: 'api', keywords: 'cframe position rotation matrix lookvector angles lookat transform right up' },
    { title: 'Vector3', url: 'https://create.roblox.com/docs/reference/engine/datatypes/Vector3', snippet: 'Vektor 3D. Vector3.new(x,y,z). X, Y, Z, Magnitude, Unit. Lerp, dot, cross.', category: 'api', keywords: 'vector3 xyz magnitude unit lerp dot cross new math direction force' },
    { title: 'Color3', url: 'https://create.roblox.com/docs/reference/engine/datatypes/Color3', snippet: 'Warna RGB. Color3.new(r,g,b), Color3.fromRGB(r,g,b). R/G/B antara 0-1.', category: 'api', keywords: 'color3 rgb fromrgb fromhsv colour warna' },
    ...
    { title: 'Script / LocalScript / ModuleScript', url: 'https://create.roblox.com/docs/reference/engine/classes/Script', snippet: 'Script: server. LocalScript: client. ModuleScript: require() dari keduanya.', category: 'api', keywords: 'script localscript modulescript server client source enabled require' },
    { title: 'RemoteEvent & RemoteFunction', url: 'https://create.roblox.com/docs/reference/engine/classes/RemoteEvent', snippet: 'Komunikasi server-client. OnServerEvent, OnClientEvent, FireServer, FireClient, FireAllClients. InvokeServer, InvokeClient.', category: 'api', keywords: 'remoteevent remotefunction onserverevent onclientevent fireserver fireclient fireallclients invoke' },
    { title: 'BindableEvent & BindableFunction', url: 'https://create.roblox.com/docs/reference/engine/classes/BindableEvent', snippet: 'Komunikasi script-to-script sisi sama. Event:Fire(), Event.Event:Connect(). Function:Invoke().', category: 'api', keywords: 'bindableevent bindablefunction fire event connect invoke callback internal' },
    { title: 'Players Service', url: 'https://create.roblox.com/docs/reference/engine/classes/Players', snippet: 'Kelola pemain. PlayerAdded, PlayerRemoving, GetPlayers, LocalPlayer, GetPlayerFromCharacter.', category: 'api', keywords: 'players playeradded playerremoving getplayers localplayer character management' },
    { title: 'DataStoreService', url: 'https://create.roblox.com/docs/reference/engine/classes/DataStoreService', snippet: 'GetDataStore(name). GetAsync, SetAsync, UpdateAsync, RemoveAsync. Selalu pcall!', category: 'api', keywords: 'datastore getasync setasync updateasync removeasync save load persistent data' },
    ...
    { title: 'UserInputService', url: 'https://create.roblox.com/docs/reference/engine/classes/UserInputService', snippet: 'InputBegan, InputEnded, GetKeysPressed, IsKeyDown, MouseMoved, GetMouseLocation. LocalScript only.', category: 'api', keywords: 'userinputservice input keyboard mouse touch inputbegan inputended keycode' },
    { title: 'CollectionService', url: 'https://create.roblox.com/docs/reference/engine/classes/CollectionService', snippet: 'AddTag, RemoveTag, GetTagged(tag), HasTag, GetTags. Untuk grup sistem modular.', category: 'api', keywords: 'collectionservice tag addtag removetag gettagged hastag modular' },
    { title: 'InsertService', url: 'https://create.roblox.com/docs/reference/engine/classes/InsertService', snippet: 'LoadAsset(assetId) returns Model. Gunakan pcall. Asset harus publik.', category: 'api', keywords: 'insertservice loadasset asset insert model catalog pcall' },
    { title: 'HttpService', url: 'https://create.roblox.com/docs/reference/engine/classes/HttpService', snippet: 'GetAsync, PostAsync, JSONEncode, JSONDecode. Aktifkan di Game Settings.', category: 'api', keywords: 'httpservice getasync postasync http json encode decode webhook api' },
    { title: 'ReplicatedStorage & ServerStorage', url: 'https://create.roblox.com/docs/reference/engine/classes/ReplicatedStorage', snippet: 'ReplicatedStorage: server + client. ServerStorage: server only.', category: 'api', keywords: 'replicatedstorage serverstorage storage replicate module asset' },
    { title: 'MessagingService', url: 'https://create.roblox.com/docs/reference/engine/classes/MessagingService', snippet: 'Cross-server. PublishAsync(topic, message), SubscribeAsync(topic, callback).', category: 'api', keywords: 'messagingservice cross server publish subscribe topic broadcast' },
    ...
    { title: 'Teams Service', url: 'https://create.roblox.com/docs/reference/engine/classes/Teams', snippet: 'game:GetService("Teams"). Create Team dengan BrickColor. Player.Team untuk assign.', category: 'api', keywords: 'teams service team brickcolor player assign color' },
    ...
    { title: 'StarterGui / StarterPlayer', url: 'https://create.roblox.com/docs/reference/engine/classes/StarterGui', snippet: 'StarterGui: GUI awal player. StarterPlayer: script awal. StarterCharacterScripts, StarterPlayerScripts.', category: 'api', keywords: 'startergui starterplayer startercharacterscripts starterplayerscripts gui character' },
    { title: 'ScreenGui / Frame / TextLabel / TextButton / TextBox', url: 'https://create.roblox.com/docs/reference/engine/classes/ScreenGui', snippet: 'GUI 2D. Size, Position, BackgroundColor3, TextColor3, Font, Visible.', category: 'api', keywords: 'screengui frame textlabel textbutton textbox gui udim2 backgroundcolor3 textcolor3 font' },
    ...
    { title: 'BillboardGui & SurfaceGui', url: 'https://create.roblox.com/docs/reference/engine/classes/BillboardGui', snippet: 'GUI menempel di Part. BillboardGui menghadap kamera. SurfaceGui di permukaan.', category: 'api', keywords: 'billboardgui surfacegui part attach face camera 3d npc overhead' },
    { title: 'UIListLayout / UIGridLayout', url: 'https://create.roblox.com/docs/reference/engine/classes/UIListLayout', snippet: 'Atur posisi child otomatis. Padding, FillDirection, SortOrder.', category: 'api', keywords: 'uilistlayout uigridlayout layout padding filldir horizontal vertical grid' },
    { title: 'UICorner / UIStroke / UIPadding / UIGradient', url: 'https://create.roblox.com/docs/reference/engine/classes/UICorner', snippet: 'Styling. UICorner: sudut bulat. UIStroke: border. UIPadding: margin. UIGradient.', category: 'api', keywords: 'uicorner uistroke uipadding uigradient corner border margin gradient rounded' },
    ...
    { title: 'Lighting Service', url: 'https://create.roblox.com/docs/reference/engine/classes/Lighting', snippet: 'Ambient, Brightness, ClockTime, FogEnd, FogStart. Child: Sky, Atmosphere, BloomEffect.', category: 'api', keywords: 'lighting ambient brightness clock fog sky atmosphere bloom environment' },
    { title: 'Constraints (Hinge, Spring, Rope)', url: 'https://create.roblox.com/docs/reference/engine/classes/HingeConstraint', snippet: 'Connect Part via Attachment. HingeConstraint, SpringConstraint, RopeConstraint, WeldConstraint.', category: 'api', keywords: 'constraint hinge spring rope rod weld attachment physics joint motor' },
    ...
    { title: 'Luau — task Library', url: 'https://create.roblox.com/docs/reference/engine/libraries/task', snippet: 'task.wait(n), task.spawn(fn), task.delay(t,fn), task.cancel(thread). Lebih akurat dari wait().', category: 'guide', keywords: 'task wait spawn delay cancel coroutine thread async timing yield' },
    { title: 'Luau — Type Checking', url: 'https://create.roblox.com/docs/luau/types', snippet: 'local x: number = 5. Types: string, number, boolean, nil, any. typeof() runtime check.', category: 'guide', keywords: 'luau type typing annotation number string boolean typeof checking inference' },
    ...
    { title: 'Luau — pcall & xpcall', url: 'https://create.roblox.com/docs/luau/functions#pcall', snippet: 'pcall(fn, ...) → ok, result. Selalu pcall untuk DataStore, HTTP, InsertService.', category: 'guide', keywords: 'pcall xpcall error handler try catch protection safe' },
    ...
    { title: 'DataStore — Best Practices', url: 'https://create.roblox.com/docs/cloud/open-cloud/data-store-api-handling', snippet: 'Selalu pcall. UpdateAsync lebih aman dari SetAsync. Retry dengan exponential backoff. SessionLocking.', category: 'guide', keywords: 'datastore best practice updateasync retry session lock save load player data' },
    { title: 'Remote Events — Best Practices', url: 'https://create.roblox.com/docs/scripting/events/remote', snippet: 'Validasi input di server. Jangan percaya data dari client. Gunakan RemoteFunction sparingly.', category: 'guide', keywords: 'remote event best practice security validate server client trust exploit' },
    { title: 'ProfileService (3rd party)', url: 'https://github.com/MadStudioRoblox/ProfileService', snippet: 'Library DataStore populer. ProfileStore:LoadProfileAsync(key). Profile:Get/Set. Session-locked.', category: 'guide', keywords: 'profileservice datastore third party library load save session lock popular' },
    { title: 'Knit Framework', url: 'https://sleitnick.github.io/Knit/', snippet: 'Framework Roblox. Knit.CreateService, Knit.CreateController. Server/client communication otomatis.', category: 'guide', keywords: 'knit framework service controller roblox architecture module' },
    { title: 'Enum Reference', url: 'https://create.roblox.com/docs/reference/engine/enums', snippet: 'Enum.Material, Enum.KeyCode, Enum.Font, Enum.SortOrder, Enum.HorizontalAlignment. Gunakan Enum.X.Y.', category: 'api', keywords: 'enum material keycode font sortorder horizontal vertical alignment fill direction' },
    ...
    { title: 'Pathfinding Service', url: 'https://create.roblox.com/docs/reference/engine/classes/PathfindingService', snippet: 'CreatePath(), ComputeAsync(start, end), GetWaypoints(). AgentParameters: radius, height, jumpHeight.', category: 'api', keywords: 'pathfinding npc navigation ai moveto waypoints compute agent' },
    { title: 'ReplicaService / Replica (3rd party)', url: 'https://madstudioroblox.github.io/ReplicaService/', snippet: 'State replication library. ReplicaServer, ReplicaController. Data berubah otomatis terefleksi ke client.', category: 'guide', keywords: 'replicaservice replica state replication library server client sync' },
    { title: 'Spring Module (SyncedPoser)', url: 'https://devforum.roblox.com/t/spring-module/237126', snippet: 'Animasi spring fisika. Spring.new(mass, force, damping, speed). Spring:shove(vector).', category: 'guide', keywords: 'spring module animation camera sway bob damping mass physics' },
    ...
    { title: 'MemoryStoreService', url: 'https://create.roblox.com/docs/reference/engine/classes/MemoryStoreService', snippet: 'Fast temporary storage per-server. GetSortedMap, GetQueue. SetAsync, GetAsync. TTL max 45 hari.', category: 'api', keywords: 'memorystoreservice fast cache temporary sorted map queue cross server' },
    { title: 'Roblox TypeScript (roblox-ts)', url: 'https://roblox-ts.com/', snippet: 'TypeScript untuk Roblox. npm install. tsconfig.json. types: @rbxts/types. rbxts compile.', category: 'guide', keywords: 'typescript roblox-ts ts npm compile types rbxts' },
  ];
}
...
function authorizeCommand(req, senderUser, targetUser, action) {
  ...
  if (senderUser !== targetUser)
    return { ok: false, status: 403, error: 'Forbidden: Hanya bisa menargetkan session sendiri.' };
  if (action && ADMIN_ONLY_ACTIONS.has(action))
    return { ok: false, status: 403, error: `Forbidden: "${escapeHtml(action, 60)}" memerlukan Admin Token.` };
  const candidate =
  ...
  switch (result) {
    ...
    case 'place_mismatch': return { ok: false, status: 403, error: 'PlaceId tidak cocok.' };
    default:             return { ok: false, status: 401, error: 'Session token tidak valid.' };
  }
}
...
export default async function handler(req, res) {
  try {
    ...
    return res.status(405).json({ error: 'Method tidak diizinkan.', allowed: ['GET', 'POST', 'OPTIONS'] });
  } catch (err) {
    ...
    try {
      return res.status(500).json({
        ...
        version: WEB_VERSION, ts: Date.now(),
      });
    } catch (_) {}
  }
}
...
async function handleGet(req, res) {
  ...
  if (q.version === '1') {
    return res.status(200).json({
      ...
      v12_features: [
        ...
        'Session Audit Log',
        'Game Info API',
        'Avatar/User Info API (cached)',
        'Auto Queue Cleanup (>30min old commands)',
  ...
  if (q.userinfo === '1') {
    ...
    try {
      ...
      return res.status(502).json({ ok: false, error: sanStr(e?.message || 'Gagal fetch user info', 100) });
    }
  }
  ...
  if (q.gameinfo === '1') {
    ...
    if (!id) return res.status(400).json({ ok: false, error: 'Parameter id diperlukan.' });
    try {
      const info = await fetchRobloxGameInfo(id, isPlace);
      return res.status(200).json({ ok: true, ...info });
    } catch (e) {
      return res.status(502).json({ ok: false, error: sanStr(e?.message || 'Gagal fetch game info', 100) });
    }
  }
  ...
  if (q.clear_cache != null) {
    if (!verifyAdminToken(req)) return res.status(401).json({ ok: false, error: 'Admin token diperlukan.' });
    const pattern = q.pattern || null;
    cacheClear(pattern);
    return res.status(200).json({ ok: true, message: 'Cache cleared.', pattern, ts: Date.now() });
  }

  const gu = san(q.user || '');

  // ── Data Getters (same as v10 + new ones) ─────────────────────────────
  ...
  if (q.get_workspace    != null) { const d = getGameScan(gu) || readJson(wsFile(gu)); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Belum ada data.' }); }
  if (q.get_script       != null) { const d = getScriptContent(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada script.' }); }
  if (q.get_script_list  != null) { const d = getScriptList(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada list.' }); }
  if (q.get_script_lines != null) { const d = getScriptLines(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada lines.' }); }
  if (q.get_mentions     != null) { const m = getMentions(gu); return res.status(200).json({ ok: true, mentions: m, count: m.length }); }
  if (q.get_search       != null) { const d = getSearch(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada hasil.' }); }
  if (q.get_game_scan    != null) { const d = getGameScan(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada scan.' }); }
  if (q.get_descendants  != null) { const d = getDescendants(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada descendants.' }); }
  if (q.get_properties   != null) { const d = getProperties(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada properties.' }); }
  if (q.get_action_list  != null) { const d = getActionList(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada list.' }); }
  if (q.get_asset_lib    != null) { const d = getAssetLib(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada asset lib.' }); }
  if (q.get_asset_id     != null) { const d = getAssetId(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada asset id.' }); }
  if (q.get_asset_folder != null) { const d = getAssetFolder(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada folder.' }); }
  if (q.get_theme_data   != null) { const d = getThemeData(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada theme.' }); }
  if (q.get_themes_list  != null) { const d = getThemesList(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada themes.' }); }
  if (q.get_theme_applied!= null) { const d = getThemeApplied(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada applied.' }); }
  if (q.get_theme_compare!= null) { const d = getThemeCompare(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada compare.' }); }
  if (q.get_module_list  != null) { const d = getModuleList(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada modules.' }); }
  if (q.get_module_deploy!= null) { const d = getModuleDeploy(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada deploy.' }); }
  if (q.get_terrain      != null) { const d = getTerrainResult(gu); return d ? res.status(200).json({ ok: true, ...d }) : res.status(200).json({ ok: false, error: 'Tidak ada terrain.' }); }

  // ── NEW v12: get_user_cmd_history ─────────────────────────────────────
  ...
  // ── NEW v12: get logsvc with filter ───────────────────────────────────
  ...
  if (q.get_stats != null) {
    ...
    return res.status(200).json({
      ...
      totalSessions: s.totalSessions || 0, startedAt: s.startedAt || 0,
      uptime: Date.now() - (s.startedAt || Date.now()), activeSessions: sessionStore.size,
      popularActions: Object.entries(s.popularActions || {}).sort(([,a],[,b]) => b-a).slice(0, 20)
        .reduce((acc, [k, v]) => { acc[k] = v; return acc; }, {}),
    });
  }
  if (q.clear_queue != null) {
    if (!verifyAdminToken(req)) return res.status(401).json({ ok: false, error: 'Admin token diperlukan.' });
    const u = san(q.user || '');
    if (!u) return res.status(400).json({ ok: false, error: 'user diperlukan.' });
    clearQueue(u);
    return res.status(200).json({ ok: true, message: 'Queue cleared.', user: u });
  }
  if (q.get_actions != null) {
    return res.status(200).json({
      ok: true, actions: [...VALID_ACTIONS], count: VALID_ACTIONS.size,
      admin_only: [...ADMIN_ONLY_ACTIONS], categories: ACTION_CATEGORIES,
      aliases: ACTION_ALIASES, aliases_count: Object.keys(ACTION_ALIASES).length,
    });
  }
  if (q.cleanup === '1') {
    if (!verifyAdminToken(req)) return res.status(401).json({ ok: false, error: 'Admin token diperlukan.' });
  ...
  if (!pu) return res.status(400).json({ error: 'user diperlukan.', queue: [] });

  // Session registration from plugin
  if (q.session_token) {
  ...
  // Drain both queues (priority first, then normal)
...
async function handlePost(req, res) {
  ...
  if (!checkIpRateLimit(ip)) {
    return res.status(429).json({ status: 'error', error: `IP rate limit tercapai.`, retryAfter: 60 });
  }

  // User rate limit check
  if (!checkRateLimit(ratUser)) {
    return res.status(429).json({ status: 'error', error: `Rate limit: max ${RATE_LIMIT_PER_MIN} req/menit.`, retryAfter: 60 });
  }

  // Burst limit check
  if (!checkBurstLimit(ratUser)) {
    return res.status(429).json({ status: 'error', error: `Burst limit: max ${RATE_LIMIT_BURST} cmd per 5 detik.`, retryAfter: 5 });
  }

  // HMAC verification (optional)
  if (!verifyPluginHmac(req, body)) {
    return res.status(401).json({ status: 'error', error: 'Invalid HMAC signature.', hint: 'Cek PLUGIN_HMAC_SECRET.' });
  }
  ...
  if (resolvedAction === 'reset') {
    ...
    if (!target) return res.status(400).json({ error: 'user diperlukan.' });
  ...
  if (resolvedAction === 'status') {
    ...
    return res.status(200).json({
      connected:   isOnline(target), online: isOnline(target),
      lastPoll:    lastPoll(target),
      queueLength: nq.length + pq.length,
      priorityQueue: pq.length, normalQueue: nq.length,
      sessionStats:  getSessionStats(target),
      hasSession:  !!sess, placeId: sess?.placeId || null,
      required_plugin_version: REQUIRED_PLUGIN_VERSION,
      web_version: WEB_VERSION, currentProject: getProject(target),
    });
  }

  // ── Set Project ───────────────────────────────────────────────────────
  if (resolvedAction === 'set_project') {
    if (!u) return res.status(400).json({ error: 'user diperlukan.' });
  ...
  if (resolvedAction === 'set_webhook') {
    ...
    if (webhookUrl && !webhookUrl.startsWith('https://'))
      return res.status(400).json({ error: 'Webhook URL harus HTTPS.' });
    saveWebhook(u, webhookUrl);
    return res.status(200).json({ status: 'ok', webhookSet: !!webhookUrl, user: u });
  }

  // ── NEW v12: Multi-Target Command ─────────────────────────────────────
  if (resolvedAction === 'multi_target' && Array.isArray(body.targets)) {
    if (!verifyAdminToken(req)) return res.status(401).json({ error: 'Admin token diperlukan untuk multi_target.' });
  ...
  // ══════════════════════════════════════════════════════════════════════
  // PLUGIN CALLBACKS (data FROM plugin)
  // ══════════════════════════════════════════════════════════════════════

  const internalHandlers = {
    'game_scan': () => {
      const d = { data: sanObj(body.data), ts: body.ts || Date.now(), user: u };
      saveGameScan(u, d); writeJson(wsFile(u), { ...d, _ts: Date.now() });
      return res.status(200).json({ status: 'ok', ts: d.ts });
    },
    'workspace_data': () => {
      pushLog({ action: 'workspace_read', user: u }); writeJson(wsFile(u), { ...body, _ts: Date.now() });
      return res.status(200).json({ status: 'ok' });
    },
    'output_data': () => { saveOutput(u, sanArr(body.outputs)); return res.status(200).json({ status: 'ok' }); },
    'script_content': () => {
      saveScriptContent(u, { name: sanStr(body.name||'',100), parent: sanStr(body.parent||'',100), fullPath: sanStr(body.fullPath||'',200), scriptType: sanStr(body.scriptType||'Script',30), source: String(body.source||''), lineCount: sanInt(body.lineCount,0,0,99_999), disabled: !!body.disabled, updatedAt: Date.now() });
      pushLog({ action: 'script_read', user: u, name: sanStr(body.name||'',50) });
      return res.status(200).json({ status: 'ok', name: sanStr(body.name||'',50) });
    },
    'script_list': () => { saveScriptList(u, { parent: sanStr(body.parent||'',100), scripts: sanArr(body.scripts), count: sanInt(body.count,0,0,99_999), updatedAt: Date.now() }); return res.status(200).json({ status: 'ok', count: sanInt(body.count,0,0,99_999) }); },
    'script_lines': () => { saveScriptLines(u, { name: sanStr(body.name||'',100), lineStart: sanInt(body.lineStart,1,1,99_999), lineEnd: sanInt(body.lineEnd,1,1,99_999), total: sanInt(body.total,0,0,99_999), content: sanStrSafe(body.content||''), updatedAt: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'log_output': () => { const logs = sanArr(body.logs, 100); saveLogSvc(u, logs); return res.status(200).json({ status: 'ok', received: logs.length }); },
    'mention_resolved': () => { saveMention(u, { mention: sanStr(body.mention||'',100), object: sanObj(body.object), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'search_result': () => { saveSearch(u, { query: sanStr(body.query||'',200), results: sanArr(body.results), count: sanInt(body.count,0,0,99_999), ts: Date.now() }); return res.status(200).json({ status: 'ok', count: sanInt(body.count,0,0,99_999) }); },
    'descendants': () => { saveDescendants(u, { target: sanStr(body.target||'',100), descendants: sanArr(body.descendants), count: sanInt(body.count,0,0,99_999), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'object_properties': () => { saveProperties(u, { name: sanStr(body.name||'',100), properties: sanObj(body.properties), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'action_list': () => { saveActionList(u, { actions: sanArr(body.actions), count: sanInt(body.count,0,0,9_999), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'asset_library': () => { saveAssetLib(u, { category: sanStr(body.category||'all',50), data: sanObj(body.data||body.summary), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'assets_listed': () => { saveAssetLib(u, { category: sanStr(body.category||'all',50), data: sanObj(body.data||body.summary), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'asset_id_result': () => { saveAssetId(u, { category: sanStr(body.category||'',50), sub: sanStr(body.sub||'',50), name: sanStr(body.name||'',100), id: sanStr(body.id||'',100), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'asset_folder_list': () => { saveAssetFolder(u, { folder: sanStr(body.folder||'all',50), contents: sanObj(body.contents), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'theme_data': () => { saveThemeData(u, { name: sanStr(body.name||body.theme||'nexus_ai',50), label: sanStr(body.label||'',50), theme: sanObj(body.theme||body.data), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'themes_list': () => { saveThemesList(u, { themes: sanArr(body.themes), count: sanInt(body.count,0,0,999), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'theme_list': () => { saveThemesList(u, { themes: sanArr(body.themes), count: sanInt(body.count,0,0,999), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'theme_applied': () => { saveThemeApplied(u, { target: sanStr(body.target||'',100), theme: sanStr(body.theme||'',50), count: sanInt(body.count,0,0,9_999), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'theme_compare': () => { saveThemeCompare(u, { theme_a: sanObj(body.theme_a), theme_b: sanObj(body.theme_b), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'module_deployed': () => { saveModuleDeploy(u, { name: sanStr(body.name||'',100), parent: sanStr(body.parent||'',100), source: sanStr(body.source||'',100), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'modules_list': () => { saveModuleList(u, { folder: sanStr(body.folder||'modulesscripts',100), modules: sanArr(body.modules), count: sanInt(body.count,0,0,999), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
    'terrain_materials': () => { saveTerrainResult(u, { materials: sanArr(body.materials), count: sanInt(body.count,0,0,999), ts: Date.now() }); return res.status(200).json({ status: 'ok' }); },
  };
  ...
  if (resolvedAction === 'get_logs' || resolvedAction === 'get_history') {
    if (!verifyAdminToken(req)) return res.status(401).json({ error: 'Admin token diperlukan.' });
  ...
  if (resolvedAction === 'search_toolbox') {
    ...
    if (!keyword) return res.status(400).json({ status: 'error', error: '"keyword" wajib diisi.', action: 'search_toolbox' });
    ...
    try {
      ...
      if (target && isOnline(target)) {
        pushQueue(target, { action: 'search_result_toolbox', keyword, assetType: finalType, assets: result.assets.slice(0, 20), nextCursor: result.nextCursor, total: result.total, _user: sender }, 'normal');
      }
      return res.status(200).json({ status: 'ok', action: 'search_toolbox', keyword, assetType: finalType, assets: result.assets, nextCursor: result.nextCursor, total: result.total, count: result.assets.length, pluginNotified: target ? isOnline(san(body._target_user || sender)) : false, fromCache: !!cacheGet(`toolbox:${keyword}:${finalType}:${limit}:${cursor||''}`), ts: Date.now() });
    } catch (err) {
      const code = err?.code || 500;
      pushLog({ action: 'search_toolbox_error', user: sender || 'web', error: sanStr(err?.message || '', 100) });
      return res.status(code === 400 ? 400 : code === 429 ? 429 : 502).json({ status: 'error', action: 'search_toolbox', message: sanStr(err?.message || 'Gagal.', 200), code, ts: Date.now() });
    }
  }
  ...
  if (resolvedAction === 'insert_model') {
    ...
    if (!assetId) return res.status(400).json({ status: 'error', error: '"asset_id" wajib diisi.', action: 'insert_model' });
    if (!target)  return res.status(400).json({ status: 'error', error: '_user diperlukan.', action: 'insert_model' });
    ...
    try {
      ...
      if (!validated.insertable && validated.assetType !== 'Unknown')
        return res.status(400).json({ status: 'error', action: 'insert_model', message: `AssetType "${validated.assetType}" tidak bisa di-insert.`, assetType: validated.assetType, ts: Date.now() });

      pushQueue(target, { action: 'insert_rbx_model', asset_id: validated.assetId, name: validated.name, parent, insert_code: validated.insertCommand, _user: sender, _target_user: target }, priority);
      ...
      return res.status(200).json({ status: 'ok', action: 'insert_model', assetId: validated.assetId, name: validated.name, description: validated.description || '', assetType: validated.assetType, creator: validated.creator, isPublic: validated.isPublic, unverified: validated.unverified || false, insertable: validated.insertable, insertCommand: validated.insertCommand, parent, pluginConnected: isOnline(target), queued: true, priority, queueLength: getQueue(target).length, ts: Date.now() });
    } catch (err) {
      const code = err?.code || 500;
      pushLog({ action: 'insert_model_error', user: sender || 'web', error: sanStr(err?.message || '', 100) });
      return res.status(code === 400 ? 400 : 502).json({ status: 'error', action: 'insert_model', message: sanStr(err?.message || 'Gagal.', 200), ts: Date.now() });
    }
  }
  ...
  if (resolvedAction === 'search_docs') {
    ...
    if (!query) return res.status(400).json({ status: 'error', error: '"query" wajib diisi.', action: 'search_docs' });
    ...
    try {
      ...
      return res.status(500).json({ status: 'error', action: 'search_docs', message: sanStr(err?.message || 'Gagal.', 200), ts: Date.now() });
    }
  }

  // ── NEW v12: get_game_info ────────────────────────────────────────────
  if (resolvedAction === 'get_game_info') {
    ...
    if (!id) return res.status(400).json({ status: 'error', error: '"id" diperlukan.', action: 'get_game_info' });
    try {
      const info = await fetchRobloxGameInfo(id, isPlace);
      bumpStats(sender || 'web', 'get_game_info');
      return res.status(200).json({ status: 'ok', action: 'get_game_info', ...info, ts: Date.now() });
    } catch (err) {
      return res.status(502).json({ status: 'error', action: 'get_game_info', message: sanStr(err?.message || 'Gagal.', 200), ts: Date.now() });
    }
  }

  // ── NEW v12: get_user_info ────────────────────────────────────────────
  if (resolvedAction === 'get_user_info' || resolvedAction === 'get_avatar_info') {
    ...
    if (!userId) return res.status(400).json({ status: 'error', error: '"user_id" diperlukan.' });
    try {
      ...
      return res.status(502).json({ status: 'error', action: resolvedAction, message: sanStr(err?.message || 'Gagal.', 200), ts: Date.now() });
    }
  }

  // ── NEW v12: validate_asset ───────────────────────────────────────────
  if (resolvedAction === 'validate_asset') {
    const sender  = san(body._user || '');
    const assetId = body.asset_id || body.assetId || body.id || '';
    if (!assetId) return res.status(400).json({ status: 'error', error: '"asset_id" diperlukan.' });
    try {
      const validated = await validateAndPrepareAsset(assetId);
  ...
  if (resolvedAction === 'batch_commands') {
    ...
    if (!isAdmin && sender !== target)
      return res.status(403).json({ error: 'Forbidden: Tidak bisa menargetkan pengguna lain.' });
    if (!isAdmin) { const auth = authorizeCommand(req, sender, target, null); if (!auth.ok) return res.status(auth.status).json({ error: auth.error }); }
    ...
    for (const cmd of safe) {
      ...
      pushQueue(target, { ...cmd, action: act, _user: String(body._user || 'web').substring(0, 50), _target_user: target, _apiKey: undefined }, priority);
      pushed++;
    }

    bumpStats(sender || 'web', 'batch_commands');
    pushLog({ action: 'batch_commands', user: sender || 'web', target, count: pushed, skipped, priority });
    pushUserCmdHistory(sender, 'batch_commands', `${pushed} commands to ${target}`);
    dispatchWebhook(sender, 'batch_commands', { pushed, target }).catch(() => {});

    return res.status(200).json({ status: 'ok', pushed, skipped, priority, warning: removed.length > 0 ? `${removed.length} destructive actions dihapus.` : undefined, pluginConnected: isOnline(target), queueLength: getQueue(target).length, required_plugin_version: REQUIRED_PLUGIN_VERSION, ts: Date.now() });
  }

  // ── execute_json / execute_text ───────────────────────────────────────
  if ((resolvedAction === 'execute_json' || resolvedAction === 'execute_text') && (body.text || body.commands)) {
    const sender   = san(body._user || '');
    const target   = san(body._target_user || sender);
    const priority = sanPriority(body.priority);
    ...
    const isAdmin = verifyAdminToken(req);
    if (!isAdmin && sender !== target) return res.status(403).json({ error: 'Forbidden: execute tidak bisa menargetkan pengguna lain.' });
    if (!isAdmin) { const auth = authorizeCommand(req, sender, target, null); if (!auth.ok) return res.status(auth.status).json({ error: auth.error }); }
    ...
    return res.status(200).json({ status: pushed > 0 ? 'ok' : 'ok', pushed, skipped, priority, pluginConnected: isOnline(target), queueLength: getQueue(target).length, ts: Date.now() });
  }
  ...
  if (resolvedAction === 'inject_command' && body.command) {
    ...
    const act = ACTION_ALIASES[sanStr(cmd.action, 80)] || sanStr(cmd.action, 80);
    if (!VALID_ACTIONS.has(act)) return res.status(400).json({ error: `Action tidak valid: ${escapeHtml(act, 60)}` });
    const auth = authorizeCommand(req, sender, target, act);
    if (!auth.ok) return res.status(auth.status).json({ error: auth.error });
    pushQueue(target, { ...cmd, action: act, _user: String(body._user || 'web').substring(0, 50), _target_user: target, _apiKey: undefined, _session_token: undefined, _place_id: undefined }, priority);
  ...
  if (body.action || actionType) {
    ...
    if (!VALID_ACTIONS.has(act)) {
      // Suggest closest alias
      const suggestion = Object.entries(ACTION_ALIASES).find(([alias]) => alias.includes(rawAct) || rawAct.includes(alias));
      return res.status(400).json({
        error: `Action tidak valid: ${escapeHtml(act, 60)}`,
        hint: suggestion ? `Mungkin maksud kamu: "${suggestion[1]}" (alias dari "${suggestion[0]}")` : `Lihat valid_actions di GET?get_actions=1`,
        valid_actions_count: VALID_ACTIONS.size,
      });
    }
    ...
    const sender = san(body._user || '');
    const target = san(body._target_user || sender);
    if (!target) return res.status(400).json({ error: '_target_user atau _user diperlukan.' });
    ...
    return res.status(200).json({
      status: 'ok', action: act, target, priority,
      wasAlias:        act !== rawAct,
