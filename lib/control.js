// api/control.js — NEXUS AI Control API v14.1
// Fixes: HTTP 200 red-X bug, checkVersion 404, English comments,
//        removed broken/unused systems, admin-only actions enforced.
...
// Actions that require admin token — non-admins cannot queue these
...
// Brace-depth JSON object finder — extracts top-level JSON objects from mixed text
...
// ── SECURITY HEADERS ──────────────────────────────────────────────────────────
// Roblox HttpService sends no Origin header, so allow * in that case.
function setSecurityHeaders(req, res) {
  ...
  if (origin) {
    ...
    // No Origin header = Roblox plugin or server-side request
...
// ── VERSION PAYLOAD ───────────────────────────────────────────────────────────
// Single source of truth used by GET /api/version AND GET /api/control?version=1
...
// ── INJECT COMMAND RESULT PROCESSOR ──────────────────────────────────────────
// FIX: The plugin returns HTTP 200 with { status: "ok" } for success.
// The old code showed "HTTP 200" as a red error. This helper interprets it correctly.
function interpretPluginResponse(httpStatus, body) {
  // HTTP 200 is always success — regardless of body content
  if (httpStatus === 200) {
    return { ok: true, pushed: body?.pushed ?? 1, data: body };
  }
  // HTTP 2xx (201, 202, etc.) also success
  if (httpStatus >= 200 && httpStatus < 300) {
    return { ok: true, pushed: body?.pushed ?? 1, data: body };
  }
  // Anything else is a real error
  const errMsg = body?.error || body?.message || `HTTP ${httpStatus}`;
...
export default async function handler(req, res) {
  try {
    ...
    console.error('[NEXUS v14.1] Unhandled error:', err?.message || err);
    try {
...
async function handleGet(req, res) {
  ...
  // ── Version check ─────────────────────────────────────────────────────────
  // Handles: GET /api/control?version=1  AND  GET /api/version (bare, no params)
  // The plugin calls both routes — both must return HTTP 200 with version data.
  ...
  // ── Plugin Poll ───────────────────────────────────────────────────────────
  // Called by the Roblox plugin every ~2s to receive queued actions.
  // Returns HTTP 200 with { ok: true, queue: [...] }
  // The plugin checks: data.queue (array), data.required_plugin_version, data.ok === true
  if (!u) return sendError(res, 400, '"user" parameter is required.');
...
async function handlePost(req, res) {
  ...
  // FIX: All plugin callbacks return HTTP 200. The old code sometimes returned
  // 4xx for missing optional fields, causing the plugin to log error messages
  // even when the action actually succeeded.

  const pluginCallbacks = {

    // Plugin just connected — register session and return version info
    nxai_connect: () => {
    ...
    // Plugin sent workspace scan result
    game_scan: () => {
    ...
    // Plugin sent script output logs
    output_data: () => {
      saveOutput(resolvedUser, sanArr(body.outputs));
      return res.status(200).json({ ok: true, status: 'ok' });
    },

    // Plugin sent script source code
    script_content: () => {
      ...
      return res.status(200).json({ ok: true, status: 'ok', name: sanStr(body.name || '', 50) });
    },

    // Plugin sent list of scripts
    script_list: () => {
    ...
    // Plugin sent specific script lines
    script_lines: () => {
    ...
    // Plugin sent Studio output log entries
    log_output: () => {
    ...
    theme_data:       () => { saveThemeData(resolvedUser,    { name: sanStr(body.name || body.theme || 'nexus_ai', 50), label: sanStr(body.label || '', 50), theme: sanObj(body.theme || body.data) }); return res.status(200).json({ ok: true, status: 'ok' }); },
    themes_list:      () => { saveThemesList(resolvedUser,   { themes: sanArr(body.themes), count: sanInt(body.count, 0, 0, 999) }); return res.status(200).json({ ok: true, status: 'ok' }); },
    theme_list:       () => { saveThemesList(resolvedUser,   { themes: sanArr(body.themes), count: sanInt(body.count, 0, 0, 999) }); return res.status(200).json({ ok: true, status: 'ok' }); },
    theme_applied:    () => { saveThemeApplied(resolvedUser, { target: sanStr(body.target || '', 100), theme: sanStr(body.theme || '', 50), count: sanInt(body.count, 0, 0, 9_999) }); return res.status(200).json({ ok: true, status: 'ok' }); },
    theme_compare:    () => { saveThemeCompare(resolvedUser, { theme_a: sanObj(body.theme_a), theme_b: sanObj(body.theme_b) }); return res.status(200).json({ ok: true, status: 'ok' }); },
    ...
    // Plugin sent RunCode execution result
    runcode_result: () => {
      const result = {
    ...
    plugin_error: () => {
      ...
      // Always HTTP 200 — the plugin sending error reports is a success at the transport layer
      return res.status(200).json({ ok: true, status: 'ok' });
    },

    // ── inject_command ───────────────────────────────────────────────────────
    // This is the key endpoint called by chats.js autoInjectToStudio.
    // FIX: Was returning HTTP 4xx for valid commands, causing "HTTP 200 red X" in UI.
    // Now always returns HTTP 200 with { ok: true, status: "ok", pushed: N }.
    inject_command: () => {
      ...
      if (!auth.ok) {
        // Return HTTP 200 with ok:false so the plugin doesn't retry infinitely
        return res.status(200).json({
      ...
      pushLog({ action: act, user: sender || 'web', target, name: sanStr(cmd.name || '', 50) });
      pushUserHistory(sender, act, sanStr(cmd.name || '', 60));

      // HTTP 200 always — chats.js checks pushed > 0 or status === 'ok'
      return res.status(200).json({
        ok:                      true,
        status:                  'ok',
        pushed:                  pushed ? 1 : 0,
