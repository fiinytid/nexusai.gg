// api/control.js — NEXUS AI Control API  v14.0
...
export const WEB_VERSION             = 'V14.0';
...
function sanStr(str, maxLen = 200) {
  ...
  return str.replace(/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/g, '').replace(/[<>]/g, '').substring(0, maxLen);
}
...
// Robust JSON parser — handles trailing commas and Lua-style syntax
...
// Brace-depth JSON object finder
...
// FIX [5]: timing-safe compare using max length of both strings
...
// FIX [8]: filter stale on PUSH too
...
async function robloxToolboxSearch(keyword, assetType = 'Model', limit = 10, cursor = null) {
  ...
  try {
    ...
    throw Object.assign(new Error(`Roblox Toolbox connection failed: ${err?.message || 'timeout'}`), { code: 502 });
  }
...
async function validateAsset(assetId) {
  ...
  const result = {
    ...
    description: sanStr(assetData.description || '', 250), assetType: rawType,
    creator: {
...
async function fetchUserInfo(userId) {
  ...
  const result = {
    userId, username: sanStr(d.name || '', 80), displayName: sanStr(d.displayName || d.name || '', 80),
    description: sanStr(d.description || '', 300), isBanned: d.isBanned || false,
...
async function fetchGameInfo(id, isPlaceId = false) {
  ...
  const result = {
    universeId, placeId: game.rootPlaceId || parsed,
    name: sanStr(game.name || '', 120), description: sanStr(game.description || '', 500),
    creator: { name: sanStr(game.creator?.name || '', 80), type: sanStr(game.creator?.type || 'User', 20) },
    playing: game.playing || 0, visits: game.visits || 0, maxPlayers: game.maxPlayers || 0,
    favoritedCount: game.favoritedCount || 0, genre: sanStr(game.genre || '', 30),
...
async function searchDocs(query, docType = 'all', limit = 5) {
  ...
  const scored = index
    ...
    .slice(0, maxRes);
  if (scored.length === 0) {
    return { results: [{ title: 'Roblox Creator Documentation', url: 'https://create.roblox.com/docs', snippet: `No results found for "${q}".`, category: 'fallback' }], source: 'local_fallback', query: q };
  }
  const result = { results: scored.map(({ score: _s, keywords: _k, ...rest }) => rest), source: 'local_index', query: q };
...
// ── SECURITY HEADERS ─────────────────────────────────────────────────────────
// FIX [6]: Roblox HttpService sends no Origin — use * only when no origin header
...
// ── RESPONSE HELPERS ──────────────────────────────────────────────────────────
// FIX [7]: sendError always returns correct HTTP status
...
function sendRateLimit(res, retryAfter = 60) {
  return res.status(429).json({ ok: false, status: 'error', error: 'Rate limit exceeded.', retryAfter, ts: Date.now() });
}

// ── VERSION PAYLOAD ───────────────────────────────────────────────────────────
// FIX [1] [2]: single source of truth for version response used by both
// /api/version route (bare GET) and /api/control?version=1
...
function versionPayload() {
  return {
    ...
    changelog:               `v14.0 — Fixed checkVersion 404, HTTP 200 red X, English comments, dead code removed.`,
    security_model: {
...
export default async function handler(req, res) {
  try {
    ...
    console.error('[NEXUS v14.0] Unhandled error:', err?.message || err);
    try {
...
async function handleGet(req, res) {
  ...
  // FIX [1][2]: Handle checkVersion — plugin calls GET /api/version (no query params)
  // AND GET /api/control?version=1. Both must return the same version payload.
  // When Vercel routes /api/version to this file, q will be empty, so
  // Object.keys(q).length === 0 catches the bare /api/version call.
  ...
  if (q.get_webhook != null) {
    const wh = getWebhook(u);
    return res.status(200).json({ ok: true, webhook: wh ? { url: wh.url, updatedAt: wh.updatedAt } : null, user: u });
  }
  ...
  if (q.queue_stats != null) {
    ...
    return res.status(200).json({
      ...
      total: nq.length + pq.length, oldestMs: nq[0]?._ts ? Date.now() - nq[0]._ts : null,
  ...
  // ── Plugin Poll ─────────────────────────────────────────────────────────────
  // FIX [3]: poll response includes all required fields that the plugin checks.
  // Plugin checks: data.queue (array), data.required_plugin_version, data.ok === true.
  if (!u) return sendError(res, 400, '"user" parameter is required.');
...
async function handlePost(req, res) {
  ...
  const pluginCallbacks = {
    ...
    nxai_connect: () => {
      ...
      return res.status(200).json({
        ok: true, required_plugin_version: REQUIRED_PLUGIN_VERSION,
        web_version: WEB_VERSION, api_version: API_VERSION,
        currentProject: proj, projectId: proj.projectId || '', projectName: proj.projectName || '',
        ts: Date.now(),
      });
    },
    ...
    mention_resolved: () => {
      saveMention(resolvedUser, { mention: sanStr(body.mention || '', 100), object: sanObj(body.object), found: true });
      return res.status(200).json({ ok: true });
    },

    mention_not_found: () => {
      saveMention(resolvedUser, { mention: sanStr(body.mention || '', 100), object: null, found: false, ts: Date.now() });
    ...
    search_result: () => {
      saveSearch(resolvedUser, {
        query: sanStr(body.query || '', 200), results: sanArr(body.results), count: sanInt(body.count, 0, 0, 99_999),
      });
      return res.status(200).json({ ok: true, count: sanInt(body.count, 0, 0, 99_999) });
    },
    ...
    descendants: () => {
      saveDescendants(resolvedUser, {
        target: sanStr(body.target || '', 100), descendants: sanArr(body.descendants), count: sanInt(body.count, 0, 0, 99_999),
      });
      return res.status(200).json({ ok: true });
    },

    object_properties: () => {
      saveProperties(resolvedUser, { name: sanStr(body.name || '', 100), properties: sanObj(body.properties) });
      return res.status(200).json({ ok: true });
    },
    ...
    set_properties_result: () => {
      ...
      pushLog({ action: 'set_properties_result', user: resolvedUser, name: sanStr(body.name || '', 50) });
      return res.status(200).json({ ok: true });
    },

    action_list: () => {
      saveActionList(resolvedUser, { actions: sanArr(body.actions), count: sanInt(body.count, 0, 0, 9_999) });
      return res.status(200).json({ ok: true });
    },

    asset_library:   () => { saveAssetLib(resolvedUser, { category: sanStr(body.category || 'all', 50), data: sanObj(body.data || body.summary) }); return res.status(200).json({ ok: true }); },
    assets_listed:   () => { saveAssetLib(resolvedUser, { category: sanStr(body.category || 'all', 50), data: sanObj(body.data || body.summary) }); return res.status(200).json({ ok: true }); },
    asset_id_result: () => { saveAssetId(resolvedUser, { category: sanStr(body.category || '', 50), sub: sanStr(body.sub || '', 50), name: sanStr(body.name || '', 100), id: sanStr(body.id || '', 100) }); return res.status(200).json({ ok: true }); },
    asset_folder_list: () => { saveAssetFolder(resolvedUser, { folder: sanStr(body.folder || 'all', 50), contents: sanObj(body.contents) }); return res.status(200).json({ ok: true }); },

    theme_data:     () => { saveThemeData(resolvedUser,    { name: sanStr(body.name || body.theme || 'nexus_ai', 50), label: sanStr(body.label || '', 50), theme: sanObj(body.theme || body.data) }); return res.status(200).json({ ok: true }); },
    themes_list:    () => { saveThemesList(resolvedUser,   { themes: sanArr(body.themes), count: sanInt(body.count, 0, 0, 999) }); return res.status(200).json({ ok: true }); },
    theme_list:     () => { saveThemesList(resolvedUser,   { themes: sanArr(body.themes), count: sanInt(body.count, 0, 0, 999) }); return res.status(200).json({ ok: true }); },
    theme_applied:  () => { saveThemeApplied(resolvedUser, { target: sanStr(body.target || '', 100), theme: sanStr(body.theme || '', 50), count: sanInt(body.count, 0, 0, 9_999) }); return res.status(200).json({ ok: true }); },
    theme_compare:  () => { saveThemeCompare(resolvedUser, { theme_a: sanObj(body.theme_a), theme_b: sanObj(body.theme_b) }); return res.status(200).json({ ok: true }); },
    module_deployed: () => { saveModuleDeploy(resolvedUser, { name: sanStr(body.name || '', 100), parent: sanStr(body.parent || '', 100), source: sanStr(body.source || '', 100) }); return res.status(200).json({ ok: true }); },
    modules_list:   () => { saveModuleList(resolvedUser,   { folder: sanStr(body.folder || 'modulescripts', 100), modules: sanArr(body.modules), count: sanInt(body.count, 0, 0, 999) }); return res.status(200).json({ ok: true }); },
    terrain_materials: () => { saveTerrainResult(resolvedUser, { materials: sanArr(body.materials), count: sanInt(body.count, 0, 0, 999) }); return res.status(200).json({ ok: true }); },

    runcode_result: () => {
      const result = { mode: sanStr(body.mode || 'pipeline', 20), success: !!body.success, output: body.output ?? null, log: sanArr(body.log, 200), ts: Date.now() };
    ...
    expression_result: () => {
      const result = { expression: sanStr(body.expression || '', 300), result: body.result != null ? String(body.result).substring(0, 2000) : null, ts: Date.now() };
    ...
    query_result: () => {
      const result = { results: sanArr(body.results, 200), count: sanInt(body.count, 0, 0, 99_999), ts: Date.now() };
    ...
    plugin_error: () => {
      const errorEntry = {
        ...
        timestamp:  body.timestamp || Date.now(), ts: Date.now(),
      };
  ...
  if (rawAction === 'status') {
    ...
    return res.status(200).json({
      ok: true, connected: isOnline(target), lastPoll: lastPoll(target),
      queueLength: nq.length + pq.length, priorityQueue: pq.length, normalQueue: nq.length,
      sessionStats: getSessionStats(target), hasSession: !!sess, placeId: sess?.placeId || null,
      required_plugin_version: REQUIRED_PLUGIN_VERSION, web_version: WEB_VERSION,
    ...
    try {
      ...
      if (target && isOnline(target)) {
        pushQueue(target, {
          ...
          assets: result.assets.slice(0, 20), nextCursor: result.nextCursor, total: result.total, _user: sender,
        }, 'normal');
      }
      ...
      return res.status(200).json({
        ok: true, keyword, assetType, assets: result.assets,
        nextCursor: result.nextCursor, total: result.total, count: result.assets.length,
        pluginNotified: target ? isOnline(san(body._target_user || sender)) : false, ts: Date.now(),
      });
    } catch (err) {
  ...
  if (rawAction === 'insert_model') {
    ...
    try {
      ...
      return res.status(200).json({
        ok: true, assetId: asset.assetId, name: asset.name, description: asset.description || '',
        assetType: asset.assetType, creator: asset.creator, isPublic: asset.isPublic,
        unverified: asset.unverified || false, insertable: asset.insertable,
        insertScript: asset.insertScript, parent, priority,
        pluginConnected: isOnline(target), queued: true, queueLength: getQueue(target).length, ts: Date.now(),
      });
    } catch (err) {
      const code = err?.code || 500;
  ...
  if (rawAction === 'search_docs') {
    ...
    try {
      ...
      return res.status(200).json({ ok: true, query, docType, results: result.results, count: result.results.length, source: result.source, ts: Date.now() });
    } catch (err) {
  ...
  if (rawAction === 'batch_commands') {
    ...
    return res.status(200).json({
      ok: true, pushed, skipped, priority,
      warning: removed.length > 0 ? `${removed.length} admin-only action(s) removed.` : undefined,
      pluginConnected: isOnline(target), queueLength: getQueue(target).length,
      required_plugin_version: REQUIRED_PLUGIN_VERSION, ts: Date.now(),
    });
  }
  ...
  if (rawAction === 'execute_json' || rawAction === 'execute_text') {
    ...
    return res.status(200).json({
      ok: true, pushed, skipped, priority,
      pluginConnected: isOnline(target), queueLength: getQueue(target).length, ts: Date.now(),
    });
  }
  ...
  if (rawAction) {
    ...
    pushQueue(target, {
      ...body, action: act,
      _user: String(body._user || 'web').substring(0, 50), _target_user: target,
    ...
    return res.status(200).json({
      ok: true, action: act, target, priority,
      pluginConnected: isOnline(target),
      queueLength: getQueue(target).length + getPriorityQueue(target).length,
      required_plugin_version: REQUIRED_PLUGIN_VERSION, api_version: API_VERSION, ts: Date.now(),
    });
  }
