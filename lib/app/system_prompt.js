...
export default async function handler(req, res) {
  var method = req.method, body = req.body || {}, query = req.query || {};
  var ctx = method === 'GET' ? query : (body.context || body);

  try {
    // Inject context ke globals yang dibaca buildSysPrompt()
    if (ctx.username) {
      globalThis.SESSION = {
        user: {
          username:    ctx.username,
          displayName: ctx.displayName || ctx.username,
          robloxId:    ctx.robloxId || '',
          avatar:      ctx.avatar || '',
        },
        data: { plan: ctx.plan || 'free', roles: ctx.roles || [] },
      };
    }

    globalThis.S = {
      credits:            parseFloat(ctx.credits) || 0,
      plan:               ctx.plan || 'free',
      selectedTheme:      ctx.theme || ctx.selectedTheme || 'nexus_ai',
      playTestEnabled:    ctx.playTestEnabled !== false,
      playTestDuration:   parseInt(ctx.playTestDuration) || 15,
      currentProjectName: ctx.projectName || null,
    };

    globalThis.studioConnected = ctx.connected === true || ctx.connected === 'true';
    globalThis.PLUGIN_VER      = ctx.pluginVer || 'V1';
    globalThis.curLang         = ctx.lang || 'en';
    globalThis.isOwner         = function() { return ctx.isOwner === true || ctx.isOwner === 'true'; };
    globalThis.isAdmin         = function() { return ctx.isAdmin === true || ctx.isAdmin === 'true'; };

    if (method === 'GET' || method === 'POST') {
      return res.json({ success: true, prompt: buildSysPrompt() });
    }
