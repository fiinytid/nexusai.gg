...
function _normalizeCmd(obj){
  ...
  if(obj.params&&typeof obj.params==='object'&&!Array.isArray(obj.params)){
    Object.keys(obj.params).forEach(function(k){if(k!=='action'&&k!=='command'&&k!=='type')result[k]=obj.params[k];});
  }
...
function _jsonRepair(raw){
  ...
  raw=raw.replace(/:\s*'([^'\\]*)'/g,':"$1"');raw=raw.replace(/\[\s*'([^'\\]*)'/g,'["$1"');raw=raw.replace(/,\s*'([^'\\]*)'/g,',"$1"');
...
function _tryParseJson(raw){
  if(!raw||typeof raw!=='string')return null;raw=raw.trim();if(!raw)return null;
  if(raw.length>50000)return null;if(!raw.startsWith('{')&&!raw.startsWith('['))return null;
  try{return JSON.parse(raw);}catch(e){}
  var stripped=_stripLuaExpressions(raw);try{return JSON.parse(stripped);}catch(e){}
...
// ── LUA BLOCK DETECTION ───────────────────────────────────────────────────────
var _LUA_ACTION_PATTERNS=[/^\s*create_remote\s*\(/m,/^\s*inject_script\s*\(/m,/^\s*create_gui\s*\(/m,/^\s*create_frame\s*\(/m,/^\s*batch_commands\s*\(/m,/^\s*create_script\s*\(/m,/^\s*create_local_script\s*\(/m,/^\s*create_module\s*\(/m,/^\s*edit_script\s*\(/m,/^\s*create_part\s*\(/m,/^\s*set_property\s*\(/m,/^\s*create_text_label\s*\(/m,/^\s*inject_quick_script\s*\(/m,/^\s*set_lighting\s*\(/m,/^\s*create_npc\s*\(/m];

function isActionCallBlock(code){
  var count=0;
  for(var i=0;i<_LUA_ACTION_PATTERNS.length;i++){if(_LUA_ACTION_PATTERNS[i].test(code)){count++;if(count>=2)return true;}}
  if(count===1){var hasRealLua=/\bgame:GetService\b|\blocal\s+\w+\s*=\s*Instance\.new\b|\bPlayers\.LocalPlayer\b|\bscript\.Parent\b|\btask\.spawn\b|\btask\.wait\b/.test(code);if(!hasRealLua)return true;}
  return false;
}
...
function parseLuaBlocks(text){
  ...
  while((m=re.exec(text))!==null){
    var c=m[1].trim();if(c.length<10)continue;if(isActionCallBlock(c))continue;if(/--\s*Command\s*Batch\s*Start/i.test(c))continue;if(/^\s*batch_commands\s*\(\s*\{/.test(c))continue;if(/^\s*\{\s*["']?action["']?\s*[=:]\s*["']/.test(c))continue;
...
function parseJsonBlocks(text){
  ...
  while((m=re.exec(text))!==null){
    ...
    var processed=_stripLuaExpressions(raw);processed=processed.replace(/([{,\[]\s*)([a-zA-Z_][a-zA-Z0-9_]*)(\s*=\s*(?![=>]))/g,'$1"$2": ');
    ...
    if(fnMatch){
      ...
      if(fnParsed&&typeof fnParsed==='object'){
        if(fnName==='batch_commands'){var batchArr=fnParsed.commands||fnParsed.actions||(Array.isArray(fnParsed)?fnParsed:null);if(Array.isArray(batchArr)){batchArr.forEach(function(sub){var norm=_normalizeCmd(sub);if(norm)cmds.push(norm);});continue;}}
        else{var fnCmd=Object.assign({action:fnName},Array.isArray(fnParsed)?{}:fnParsed);var norm=_normalizeCmd(fnCmd);if(norm){cmds.push(norm);continue;}}
      }
    }
    ...
    if(Array.isArray(parsed))items=parsed;
      ...
      var foundArr=false;
      Object.keys(parsed).forEach(function(k){if(!foundArr&&Array.isArray(parsed[k])&&parsed[k].length>0){var first=parsed[k][0];if(first&&typeof first==='object'&&(first.action||first.command||first.type)){items=parsed[k];foundArr=true;}}});
      if(!foundArr&&Object.keys(parsed).length>0){Object.keys(parsed).forEach(function(k){if(/^[a-z_][a-z0-9_]*$/.test(k)&&typeof parsed[k]==='object'&&!Array.isArray(parsed[k])){var candidate=Object.assign({action:k},parsed[k]);if(_normalizeCmd(candidate))items.push(candidate);}});}
    }
    items.forEach(function(item){
      if(!item||typeof item!=='object')return;
      if(!Array.isArray(item)){
        if(item.batch_commands&&Array.isArray(item.batch_commands)){item.batch_commands.forEach(function(sub){var norm=_normalizeCmd(sub);if(norm)cmds.push(norm);});return;}
        if(item.commands&&Array.isArray(item.commands)){item.commands.forEach(function(sub){var norm=_normalizeCmd(sub);if(norm)cmds.push(norm);});return;}
      }
...
function _extractActionsFromLuaBlock(blockCode){
  ...
  while(pos<code.length){
    var searchStr=code.slice(pos),re=/\b([a-z_][a-z0-9_]{3,49})\s*\(\s*\{/g;re.lastIndex=0;var m=re.exec(searchStr);if(!m)break;
    var fnName=m[1];if(SKIP_FNS.has(fnName)){pos+=m.index+m[0].length;continue;}
    ...
    for(var bi=braceStart;bi<code.length;bi++){
      var ch=code[bi];
      if(!inStr&&ch==='['&&code[bi+1]==='['){inLongStr=true;bi+=1;continue;}if(inLongStr&&ch===']'&&code[bi+1]===']'){inLongStr=false;bi+=1;continue;}if(inLongStr)continue;
      if(!inStr&&(ch==='"'||ch==="'")){inStr=true;strChar=ch;continue;}if(inStr&&ch===strChar&&code[bi-1]!=='\\'){inStr=false;continue;}if(inStr)continue;
    ...
    if(body.indexOf('[[')>=0){
      var nameM=body.match(/name\s*=\s*["']([^"']+)["']/),parentM=body.match(/parent\s*=\s*["']([^"']+)["']/),typeM=body.match(/script_type\s*=\s*["']([^"']+)["']/),srcM=body.match(/source\s*=\s*\[\[([^\]]*(?:\][^\]][^\]]*)*)\]\]/);
      ...
      var stripped=_stripLuaExpressions(body),parsed2=_tryParseJson(stripped)||_tryParseJson(_jsonRepair(stripped));
      if(parsed2&&typeof parsed2==='object'&&!Array.isArray(parsed2)){var cmd2=Object.assign({action:fnName},parsed2);var norm=_normalizeCmd(cmd2);if(norm)cmds.push(norm);}
    }
    pos=bodyEnd+1;
  }
  return cmds;
...
function detectScriptParent(code){
  var c=code||'',first200=c.slice(0,200),trimmed=c.trim(),typeHint=c.match(/--\s*script_type:\s*(\w+)/i),parentHint=c.match(/--\s*parent:\s*([\w.]+)/i),type='Script',parent='ServerScriptService';
...
function makeStepLabel(cmd){
  ...
  if(a==='create_script')return'Create Script: '+nm;if(a==='create_local_script')return'Create LocalScript: '+nm;if(a==='create_module')return'Create ModuleScript: '+nm;if(a==='edit_script')return'Edit Script: '+nm;if(a==='batch_inject')return'Batch inject ('+(cmd.scripts||[]).length+')';if(a==='create_gui')return'Create GUI: '+nm;if(a==='create_frame')return'Create Frame: '+nm;if(a==='get_theme')return'Get theme: '+(cmd.theme||'nexus_ai');if(a==='apply_theme')return'Apply theme: '+nm;if(a==='create_remote')return'Create Remote: '+nm;if(a==='set_property')return'Set property: '+nm+'.'+(cmd.property||'');if(a==='set_service_property')return'Set service prop: '+(cmd.service||nm);if(a==='set_lighting')return'Set lighting';if(a==='fill_terrain')return'Fill terrain: '+(cmd.material||'');if(a==='create_part')return'Create Part: '+nm;if(a==='create_npc')return'Create NPC: '+nm;if(a==='delete_object')return'Delete: '+nm;if(a==='batch_commands')return'Batch ('+(cmd.commands||[]).length+' cmds)';if(a==='read_script')return'Read script: '+nm;if(a==='scan_workspace'||a==='request_scan')return'Scan workspace';if(a==='resolve_mention')return'Resolve @'+(cmd.name||cmd.mention||'?');if(a==='play_test'||a==='run_test')return'Start play test';if(a==='stop_test')return'Stop play test';if(a==='none')return null;
  return a+(nm?': '+nm:'');
}
...
async function fetchRetry(url,opts,tries){
  ...
  for(var i=0;i<tries;i++){
    try{
      var ctrl=new AbortController(),tid=setTimeout(function(){ctrl.abort();},12000),mergedOpts=Object.assign({},opts,{signal:ctrl.signal}),r=await fetch(url,mergedOpts);
      clearTimeout(tid);
      if(r.ok)return r;
      if(r.status===429){var waitMs=_jitter(3000*(i+1));toast(curLang==='id'?'Rate limit server, tunggu...':'Server rate limit, waiting...','var(--yellow)');await _sleep(waitMs);}
      else if(r.status>=500){if(i<tries-1)await _sleep(_jitter(1000*(i+1)));else return r;}
      else return r;
    }catch(e){
      if(_isAbortError(e)){if(opts&&opts.signal&&opts.signal.aborted)throw e;if(i<tries-1){await _sleep(_jitter(800*(i+1)));continue;}throw e;}
      if(i===tries-1)throw e;await _sleep(_jitter(800*(i+1)));
    }
  }
  return null;
}
...
async function safeFetch(bodyData,signal){
  try{
    if(bodyData&&bodyData.command&&bodyData.command.source&&bodyData.command.source.length>80000){bodyData=Object.assign({},bodyData,{command:Object.assign({},bodyData.command,{source:bodyData.command.source.slice(0,80000)+'\n-- [TRUNCATED: script too large]'})});}
  ...
  }catch(e){if(_isAbortError(e))throw e;console.warn('[NEXUS inject] safeFetch error:',e&&e.message);return null;}
}
...
async function autoInjectToStudio(aiResponse,userPrompt){
  ...
  var jsonCmds=parseAllCommands(aiResponse),luaBlocks=parseLuaBlocks(aiResponse);
  if(jsonCmds.length||luaBlocks.length){console.log('[NEXUS inject] Commands:',jsonCmds.length,'Lua blocks:',luaBlocks.length);}
  var allCmds=[];
  jsonCmds.forEach(function(cmd){if(!cmd.action||cmd.action==='none')return;allCmds.push({type:'json',cmd:cmd});});
  ...
  if(!allCmds.length){console.warn('[NEXUS inject] No commands found in AI response.');return null;}
  var hasPlayTest=jsonCmds.some(function(c){return c.action==='play_test'||c.action==='run_test';}),hasStopTest=jsonCmds.some(function(c){return c.action==='stop_test';});
  ...
  for(var pi=0;pi<planSteps.length;pi++){
    var step=planSteps[pi];if(!step.sid){doneCount++;continue;}
    ...
    if(step.type==='lua'){
      try{
        ...
        if(r2&&r2.ok){var rd2;try{rd2=await r2.json();}catch(je){rd2={};}if(rd2.status==='ok'||rd2.pushed>0){updateStep(step.sid,'done',step.info.type+': '+step.name,step.info.parent);summary.push(step.info.type+': '+step.name);}else{updateStep(step.sid,'error',(rd2.error||rd2.hint||'server rejected').slice(0,80));}}
        else if(r2){var httpErr='HTTP '+r2.status;try{var ed2=await r2.json();if(ed2&&ed2.error)httpErr=ed2.error.slice(0,80);}catch(e){}updateStep(step.sid,'error',httpErr);}
        else updateStep(step.sid,'error','No response (network error)');
      }catch(e){if(_isAbortError(e))return null;updateStep(step.sid,'error',String(e&&e.message||'unknown').slice(0,80));}
    }else if(step.type==='playtest'){
      try{var ptDur=S.playTestDuration||15;await safeFetchWithRetry({type:'batch_commands',commands:[{action:'play_test',duration:ptDur}],_user:user,_target_user:user},sig,1);await _sleep(5000);updateStep(step.sid,'done',t.testDone);}
      catch(e){if(_isAbortError(e))return null;updateStep(step.sid,'error',String(e&&e.message||'').slice(0,80));}
    }else{
      var cmd=step.cmd,a=cmd.action||'',cmdToSend=Object.assign({},cmd);
      ...
      try{
        ...
        if(r4&&r4.ok){var rd4;try{rd4=await r4.json();}catch(je){rd4={};}
          if(rd4.status==='ok'||rd4.pushed>0){
            ...
            else if(a==='read_script'||a==='scan_workspace'||a==='request_scan'){updateStep(step.sid,'info');}
            else{updateStep(step.sid,'done');var lbl2=makeStepLabel(cmd);if(lbl2)summary.push(lbl2);}
          }else{updateStep(step.sid,'error',(rd4.error||rd4.hint||'action rejected').slice(0,80));}
        }else if(r4){var errTxt='HTTP '+r4.status;try{var ed4=await r4.json();if(ed4&&ed4.error)errTxt=ed4.error.slice(0,80);}catch(e){}updateStep(step.sid,'error',errTxt);}
        else updateStep(step.sid,'error','No response (check network)');
      }catch(e){if(_isAbortError(e))return null;updateStep(step.sid,'error',String(e&&e.message||'unknown').slice(0,80));}
    }
    doneCount++;if(cntEl)cntEl.textContent='('+doneCount+'/'+planSteps.length+')';if(pi<planSteps.length-1)await _sleep(180);
  }
...
function _loadSysPromptScript(){
  ...
  _sysPromptLoadPromise=new Promise(function(resolve){
    ...
    var script=document.createElement('script');script.src='/js/system_prompt.js?_v='+(Date.now()-Date.now()%60000);script.async=false;
...
async function send(){
  if(S.gen)return;
  var inp=document.getElementById('inp'),txt=(inp?inp.value.trim():''),attachments=S.attachments.slice();
  ...
  S.gen=true;if(S.cancelCtrl){try{S.cancelCtrl.abort();}catch(ex){}}
  S.cancelCtrl=new AbortController();_playTestActive=false;
  ...
  var apiMsgs=msgs.slice(0,-1),sysMain=sysPrompt,sysOverflow='',SYS_CAP=7500;
  ...
  apiMsgs.push(lastM);
  var aiText='',_localCancelCtrl=S.cancelCtrl;
  try{
    var aiCtrl=new AbortController(),aiTimeoutId=setTimeout(function(){aiCtrl.abort();},90000);
  ...
  if(!isOwner()&&!isAdmin()&&aiText&&!hasError){
    var _cmds=parseAllCommands(aiText),_luas=parseLuaBlocks(aiText),_numActions=Math.max(1,_cmds.length+_luas.length),_baseCost=S.model.cost||0,_totalCost=parseFloat((_baseCost*(1+(_numActions-1)*0.1)).toFixed(2));
  ...
  if(studioConnected&&!hasError){
    ...
    var _preCmds=parseAllCommands(aiText),_preLuas=parseLuaBlocks(aiText),_hasAnything=(_preCmds.length>0||_preLuas.length>0);
...
function openAvatarModal(){if(!SESSION)return;var u=SESSION.user;document.getElementById('avatarModalName').textContent='@'+(u.username||'-');var imgEl=document.getElementById('avatarModalImg');if(imgEl){imgEl.src=u.avatar||_NEXUS_ICON_SVG;imgEl.onerror=function(){this.src=_NEXUS_ICON_SVG;};}document.getElementById('avatarModalRole').textContent=isOwner()?'Owner':isAdmin()?'Admin':'Roblox Developer';document.getElementById('avatarModalId').textContent='Roblox ID: '+(u.robloxId||'-');document.getElementById('avatarModal').classList.add('show');}
...
function openShareModal(){var cv=S.convs.find(function(x){return x.id===S.curConv;});if(!cv)return;var text='';(cv.msgs||[]).forEach(function(m){var name=m.role==='user'?('@'+(SESSION&&SESSION.user&&SESSION.user.username||'You')):'NEXUS AI';var time=m.time?new Date(m.time).toLocaleTimeString(curLang==='id'?'id-ID':'en-US',{hour:'2-digit',minute:'2-digit'}):'';text+='['+time+'] '+name+':\n'+(m.content||'')+'\n\n';});var ta=document.getElementById('shareModalTa');if(ta)ta.value=text;document.getElementById('shareModal').classList.add('show');}
...
async function sendReport(){
  ...
  try{
    ...
    if(st)st.textContent=curLang==='id'?'Terkirim!':'Sent!';if(ta)ta.value='';
...
async function redeemCode(){
  ...
  if(!inp||!inp.value.trim())return;if(!checkClientRateLimit('redeem',3))return;
  var code=inp.value.trim().toUpperCase().replace(/[^A-Z0-9\-_]/g,'');if(btn)btn.disabled=true;
  try{
...
function initThemePicker(){var t=S.selectedTheme||'nexus_ai',acc=THEME_ACCENTS[t]||THEME_ACCENTS.nexus_ai;var swatch=document.getElementById('themeSwatchBtn'),label=document.getElementById('themePickerLabel');if(swatch){if(t==='custom'){swatch.style.background='transparent';swatch.style.border='1.5px dashed var(--dim)';}else{swatch.style.background=acc.color;swatch.style.border='1px solid rgba(255,255,255,.2)';}}if(label)label.textContent=t;}
...
async function initApp(){
  ...
  if(S.currentProjectId){
    ...
    if(!S.currentProjectName&&SESSION.data&&SESSION.data.projects){var proj=SESSION.data.projects.find(function(x){return x.id===S.currentProjectId;});if(proj)S.currentProjectName=proj.name;}
  }
  updateProjectUI();
  var u=SESSION.user;
...
// ─── SERVER-SIDE EXPORT ───────────────────────────────────────────────────────
const _convStore = new Map();
function _genId() { return Date.now().toString(36) + Math.random().toString(36).slice(2,6); }
function _key(uid, cid) { return uid + ':' + cid; }
function _get(uid, cid) { return _convStore.get(_key(uid, cid)) || null; }
function _save(uid, conv) { _convStore.set(_key(uid, conv.id), Object.assign({}, conv, { updatedAt: Date.now() })); }
function _del(uid, cid) { _convStore.delete(_key(uid, cid)); }
function _list(uid) {
  var out = [];
  _convStore.forEach(function(v, k) { if (k.startsWith(uid + ':')) out.push(v); });
  return out.sort(function(a, b) { return (b.updatedAt || 0) - (a.updatedAt || 0); });
}

export default async function handler(req, res) {
  var method = req.method, body = req.body || {}, query = req.query || {}, headers = req.headers || {};
  var userId = headers['x-user-id'] || headers['x-username'] || query.userId || query.user || 'anon';

  try {
    if (method === 'GET') {
      if (query.convId) {
        var conv = _get(userId, query.convId);
        if (!conv) return res.status(404).json({ error: 'Conversation tidak ditemukan.' });
        return res.json({ success: true, conv: conv });
      }
      return res.json({ success: true, convs: _list(userId) });
    }

    if (method === 'POST') {
      var action = body.action, message = body.message, convId = body.convId, role = body.role || 'user';

      if (action === 'detect') {
        if (!message) return res.status(400).json({ error: 'Field "message" wajib diisi.' });
        return res.json({ success: true, type: detectType(message) });
      }

      if (action === 'build') {
        var src = body.msgs || (_get(userId, convId || '') || {}).msgs || [];
        var apiMsgs = src.slice(-28).map(function(m) {
          return { role: m.role === 'user' ? 'user' : 'model', content: m._rawContent || m.content || '' };
        });
        return res.json({ success: true, apiMsgs: apiMsgs });
      }

      if (action === 'clear') {
        if (!convId) return res.status(400).json({ error: 'Field "convId" wajib diisi.' });
        var c = _get(userId, convId);
        if (c) { c.msgs = []; _save(userId, c); }
        return res.json({ success: true });
      }

      if (!message) return res.status(400).json({ error: 'Field "message" wajib diisi.' });
      var id = convId || _genId();
      var conv = _get(userId, id) || { id: id, userId: userId, title: message.slice(0, 60), msgs: [], createdAt: Date.now() };
      var msgObj = { id: _genId(), role: role, content: message, type: detectType(message), time: Date.now() };
      conv.msgs.push(msgObj);
      _save(userId, conv);
      var apiMsgs = conv.msgs.slice(-28).map(function(m) {
        return { role: m.role === 'user' ? 'user' : 'model', content: m._rawContent || m.content || '' };
      });
      return res.json({ success: true, convId: id, msg: msgObj, apiMsgs: apiMsgs });
    }

    if (method === 'DELETE') {
      if (!query.convId) return res.status(400).json({ error: 'Query param "convId" wajib diisi.' });
      _del(userId, query.convId);
      return res.json({ success: true });
    }
