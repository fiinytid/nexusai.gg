...
// ── GLOBAL STATE ──────────────────────────────────────────────────────────────
var curLang = typeof window !== 'undefined' ? (localStorage.getItem('nexus_lang') || 'id') : 'id';
var SESSION = null;
...
function renderSuggestions() {
  var t = T();
  var grid = document.getElementById('suggGrid');
  if (!grid) return;
  grid.innerHTML = t.suggs.map(function (s) {
    return '<div class="sugg" onclick="useSugg(this.dataset.q)" data-q="' + esc(s.q) + '">' +
      '<div class="sugg-title"><svg viewBox="0 0 24 24">' + s.icon + '</svg>' + esc(s.title) + '</div>' +
      esc(s.body) + '</div>';
  }).join('');
}
...
function applyLang() {
  ...
  var steps = document.querySelectorAll('.install-txt');
  t.installSteps.forEach(function (txt, i) { if (steps[i]) steps[i].innerHTML = txt; });
  ...
  var mhdr = document.getElementById('mentionHdrTxt');
  if (mhdr) mhdr.textContent = curLang === 'id' ? 'Scripts & Objek di Place' : 'Scripts & Objects in Place';
  var aiP = document.getElementById('guiAIPrompt');
  if (aiP) aiP.placeholder = curLang === 'id'
    ? 'contoh: Shop GUI 3 item, scroll list, tombol beli...'
    : 'e.g. Shop GUI with 3 items, scroll list, buy button...';
...
function renderConvs() {
  ...
  if (!S.convs || !S.convs.length) {
    list.innerHTML = '<div class="conv-empty">' + t.noConv + '</div>';
    return;
  }
  list.innerHTML = S.convs.slice().reverse().map(function (cv) {
    var act = cv.id === S.curConv ? 'act' : '';
    var time = cv.time ? new Date(cv.time).toLocaleDateString(
      curLang === 'id' ? 'id-ID' : 'en-US', { day: '2-digit', month: '2-digit' }
    ) : '';
    return '<div class="ci ' + act + '" onclick="loadConv(\'' + cv.id + '\')">' +
      '<div class="ci-title">' + esc(cv.title || (curLang === 'id' ? 'Percakapan' : 'Chat')) + '</div>' +
      '<div class="ci-time">' + time + '</div>' +
      '<button class="ci-del" onclick="delConv(event,\'' + cv.id + '\')" title="Delete">x</button></div>';
  }).join('');
}

// FIX: newChat — properly reset gen state before creating new chat
function newChat() {
  if (S.gen) { _resetGenState(); }
  var id = 'c' + Date.now();
  var cv = {
    id: id,
    title: curLang === 'id' ? 'Percakapan Baru' : 'New Chat',
    time: Date.now(),
    msgs: [],
  ...
  renderConvs();
  renderMsgs([]);
  var ti = document.getElementById('chatTitle');
...
// FIX: loadConv — properly cancel ongoing gen before switching
function loadConv(id) {
  if (S.gen && S.curConv !== id) { _resetGenState(); }
  var cv = S.convs.find(function (x) { return x.id === id; });
  if (!cv) return;
  S.curConv = id;
  renderConvs();
  var ti = document.getElementById('chatTitle');
  if (ti) ti.textContent = cv.title || (curLang === 'id' ? 'Percakapan' : 'Chat');
  var w = document.getElementById('welcome');
  if (w) w.style.display = (cv.msgs && cv.msgs.length) ? 'none' : 'flex';
  renderMsgs(cv.msgs || []);
  S.attachments = [];
  renderAttachRow();
  var inp = document.getElementById('inp');
  if (inp) {
    inp.value = S.draftText[id] || '';
...
function delConv(e, id) {
  e.stopPropagation();
  S.convs = S.convs.filter(function (x) { return x.id !== id; });
  if (S.curConv === id) {
    if (S.convs.length) loadConv(S.convs[S.convs.length - 1].id);
    else newChat();
  }
  renderConvs();
  saveS();
...
function setConvTitle(convId, firstMsg) {
  var cv = S.convs.find(function (x) { return x.id === convId; });
  if (!cv) return;
  cv.title = firstMsg.slice(0, 45) + (firstMsg.length > 45 ? '..' : '');
  var ti = document.getElementById('chatTitle'); if (ti) ti.textContent = cv.title;
  renderConvs();
}
...
function getProjectName(pid) {
  if (!pid) return null;
  var projs = S.projects || SESSION && SESSION.data && SESSION.data.projects || [];
  var p = projs.find(function (x) { return x.id === pid; });
  return p ? p.name : null;
}

function updateProjectUI() {
  var n = S.currentProjectName;
  var chip = document.getElementById('sbProjChip'),
    cn = document.getElementById('sbProjName'),
    badge = document.getElementById('hdrProjBadge');
  if (n) {
...
function getFreeModel() {
  for (var i = 0; i < MODEL_LIST.length; i++) {
    if (MODEL_LIST[i].id && !MODEL_LIST[i].grp) return MODEL_LIST[i];
  }
  return { id: 'gemini-3.1-flash-lite', prov: 'gemini', cost: 0, label: 'Gemini 3.1 Flash Lite' };
}

function updateModelUI() {
  var m = S.model || getFreeModel();
  var el = document.getElementById('inpMName'); if (el) el.textContent = m.label || m.id;
  var b = document.getElementById('inpMBadge');
  if (b) {
    b.textContent = m.cost <= 0 ? 'FREE' : m.cost + ' CR';
    b.style.color = m.cost <= 0 ? 'var(--green)' : m.cost <= 1 ? 'var(--cyan)' : m.cost <= 3 ? 'var(--yellow)' : 'var(--pink)';
  }
  var ic = document.getElementById('inpMIcon');
  if (ic) { ic.src = m.icon || ''; ic.style.display = m.icon ? '' : 'none'; }
  var gm = S.guiModel || getFreeModel();
  var gel = document.getElementById('guiMName'); if (gel) gel.textContent = gm.label || gm.id;
  var gb = document.getElementById('guiMBadge');
  if (gb) {
    gb.textContent = gm.cost <= 0 ? 'FREE' : gm.cost + ' CR';
    gb.style.color = gm.cost <= 0 ? 'var(--green)' : 'var(--cyan)';
  }
  var gi = document.getElementById('guiMIcon');
  if (gi) { gi.src = gm.icon || ''; gi.style.display = gm.icon ? '' : 'none'; }
}

function buildMDDHtml(forGui) {
  var curId = forGui ? S.guiModel.id : S.model.id;
  var html = '';
  MODEL_LIST.forEach(function (m) {
    if (m.grp) { html += '<div class="mg">' + esc(m.grp) + '</div>'; return; }
    var act = m.id === curId;
    var bc = m.cost <= 0 ? 'f' : m.badge === 'BEST' ? 'p' : 's';
    var iconHtml = m.icon
    ...
    html += '<div class="mo' + (act ? ' act' : '') + '" onclick="selModel(\'' + m.id + '\',' + (forGui ? 'true' : 'false') + ')">' +
      '<div class="mo-icon">' + iconHtml + '</div>' +
      '<div><div class="mo-n">' + esc(m.label || m.id) + '</div>' +
      '<div class="mo-s">' + (m.cost <= 0 ? 'Free' : m.cost + ' CR/msg') + '</div></div>' +
...
function selModel(id, forGui) {
  var m = MODEL_LIST.find(function (x) { return x.id === id; });
  if (!m || m.grp) return;
  if (forGui) S.guiModel = m; else S.model = m;
  updateModelUI();
  var dd = document.getElementById(forGui ? 'guiMDD' : 'mDD');
...
function renderMsgs(msgs) {
  ...
  if (!msgs || !msgs.length) {
    if (w) w.style.display = 'flex';
    c.querySelectorAll('.msg,.steps-wrap').forEach(function (el) { el.remove(); });
    return;
  }
  if (w) w.style.display = 'none';
  c.querySelectorAll('.msg,.steps-wrap').forEach(function (el) { el.remove(); });
  msgs.forEach(function (m) { appendMsg(m, true); });
...
function mkAv(role) {
  ...
  if (role === 'ai') {
    ...
    img.onerror = function () {
      ...
      } catch (ex) { console.warn('[NEXUS] avatar onerror:', ex && ex.message); }
    };
    av.appendChild(img);
  } else if (SESSION && SESSION.user && SESSION.user.avatar) {
    var img2 = document.createElement('img');
    img2.src = SESSION.user.avatar; img2.alt = 'U';
    var _avRef2 = av;
    var _fallbackLetter = (SESSION && SESSION.user && SESSION.user.username) ? SESSION.user.username.charAt(0).toUpperCase() : 'U';
...
function getLangLabel(lang) {
  var map = { lua: 'Lua', luau: 'Luau', js: 'JavaScript', javascript: 'JavaScript', ts: 'TypeScript', python: 'Python', py: 'Python', html: 'HTML', css: 'CSS', json: 'JSON', bash: 'Bash', sh: 'Shell' };
  return map[(lang || '').toLowerCase()] || lang || 'Code';
}

function getFileExt(lang) {
  var map = { lua: 'lua', luau: 'lua', js: 'js', ts: 'ts', py: 'py', python: 'py', html: 'html', css: 'css', json: 'json', bash: 'sh', sh: 'sh' };
...
function appendMsg(m, skipScroll) {
  ...
  mbWrap.className = 'mb-wrap';
  var sender = document.createElement('div');
  sender.className = 'msg-sender';
  var t2 = new Date(m.time || Date.now());
  sender.innerHTML = '<span>' + (isUser ? '@' + (SESSION && SESSION.user && SESSION.user.username || 'You') : 'NEXUS AI') + '</span>' +
    '<span>' + t2.toLocaleTimeString(curLang === 'id' ? 'id-ID' : 'en-US', { hour: '2-digit', minute: '2-digit' }) + '</span>';
  ...
  if (m.attachments && m.attachments.length) {
    ...
    imgRow.className = 'msg-imgs';
    m.attachments.forEach(function (a) {
      if (a.type === 'image') {
        var img = document.createElement('img');
        img.className = 'msg-img';
        img.src = a.preview || ('data:' + (a.mime || 'image/png') + ';base64,' + a.data);
        img.alt = a.name || 'img';
  ...
  // Content — code blocks + markdown
  var content = String(m.content || '');
  if (m.studioSummary) content = stripAllCode(content);
  var codeRe = /```(\w*)\n?([\s\S]*?)```/g, codeBlocks = [];
  var processed = content.replace(codeRe, function (match, lang, code) {
    var l = (lang || '').toLowerCase();
    if (l === 'json' || m.studioSummary) return '';
  ...
  processed.split(/(%%CB_\d+%%)/).forEach(function (part) {
    ...
    if (cm) {
      var cb = codeBlocks[parseInt(cm[1])];
      if (!cb) return;
      var w2 = document.createElement('div');
      w2.className = 'code-block-wrap';
      var langBar = document.createElement('div');
      langBar.className = 'code-lang-bar';
      var btns = document.createElement('div');
      btns.className = 'code-btns';
      btns.innerHTML =
        '<button class="cbtn" onclick="copyCode(this)"><svg viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copy</button>' +
        '<button class="cbtn dl" onclick="downloadCode(this,\'' + cb.lang + '\')"><svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Download</button>';
      langBar.innerHTML = '<span>' + esc(getLangLabel(cb.lang)) + '</span>';
      langBar.appendChild(btns);
      w2.appendChild(langBar);
      var pre = document.createElement('pre');
      var codeEl = document.createElement('code');
      codeEl.className = cb.lang ? 'language-' + cb.lang : '';
      codeEl.textContent = cb.code;
  ...
  if (!isUser) {
    ...
    acts.innerHTML =
      '<button class="mab" onclick="copyMsgText(this)"><svg viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg></button>' +
      '<button class="mab" onclick="retryMsg(this)"><svg viewBox="0 0 24 24"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 102.13-9.36L1 10"/></svg></button>' +
      '<button class="mab ' + (m._liked ? 'liked' : '') + '" onclick="likeMsg(this,true)"><svg viewBox="0 0 24 24"><path d="M14 9V5a3 3 0 00-3-3l-4 9v11h11.28a2 2 0 002-1.7l1.38-9a2 2 0 00-2-2.3zM7 22H4a2 2 0 01-2-2v-7a2 2 0 012-2h3"/></svg></button>' +
      '<button class="mab ' + (m._disliked ? 'disliked' : '') + '" onclick="likeMsg(this,false)"><svg viewBox="0 0 24 24"><path d="M10 15v4a3 3 0 003 3l4-9V2H5.72a2 2 0 00-2 1.7l-1.38 9a2 2 0 002 2.3zm7-13h2.67A2.31 2.31 0 0122 4v7a2.31 2.31 0 01-2.33 2H17"/></svg></button>' +
      '<button class="mab" onclick="openShareModal()"><svg viewBox="0 0 24 24"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg></button>';
    mbWrap.appendChild(acts);
  }

  wrap.appendChild(mbWrap);
  c.appendChild(wrap);
...
function copyCode(btn) {
  ...
  if (pre) navigator.clipboard.writeText(pre.textContent).then(function () { toast(T().copiedToast); });
}
function downloadCode(btn, lang) {
  ...
  a.download = 'nexus_code.' + getFileExt(lang);
  a.click();
}
function copyMsgText(btn) {
  var b = btn.closest('.mb-wrap').querySelector('.bubble');
  if (b) navigator.clipboard.writeText(b.innerText || b.textContent).then(function () { toast(T().copiedToast); });
}
...
function createStepsCard() {
  ...
  if (w) w.style.display = 'none';
  var wrap = document.createElement('div');
  wrap.className = 'steps-wrap'; wrap.id = 'stepsWrap';
  wrap.appendChild(mkAv('ai'));
  var mbW = document.createElement('div'); mbW.className = 'mb-wrap';
  var sender = document.createElement('div'); sender.className = 'msg-sender';
  sender.innerHTML = '<span>NEXUS AI</span><span>' +
    new Date().toLocaleTimeString(curLang === 'id' ? 'id-ID' : 'en-US', { hour: '2-digit', minute: '2-digit' }) + '</span>';
  mbW.appendChild(sender);
  var box = document.createElement('div'); box.className = 'steps-box';
  var hdr = document.createElement('div'); hdr.className = 'steps-hdr'; hdr.id = 'stepsHdr';
  var spinner = document.createElement('div'); spinner.className = 'steps-hdr-spinner'; spinner.id = 'stepsSpinner';
  var hdrTxt = document.createElement('span'); hdrTxt.className = 'steps-hdr-txt'; hdrTxt.id = 'stepsTxt';
  ...
  var cancelDiv = document.createElement('div'); cancelDiv.className = 'steps-cancel'; cancelDiv.id = 'stepsCancel';
  var cb = document.createElement('button'); cb.className = 'steps-cancel-btn';
  cb.textContent = T().cancel; cb.onclick = cancelGen;
  cancelDiv.appendChild(cb);
  box.appendChild(cancelDiv);
  mbW.appendChild(box);
  wrap.appendChild(mbW);
  c.appendChild(wrap);
  _stepsEl = wrap; _stepsList = list;
  _stepsMap.clear(); _stepsId = 0;
  c.scrollTop = c.scrollHeight;
}

function removeStepsCard() {
  var el = document.getElementById('stepsWrap');
  if (el) el.remove();
  _stepsEl = null; _stepsList = null;
  _stepsMap.clear(); _stepMeta.clear();
}

function clearSteps() {
  if (!_stepsList) return;
  _stepsList.innerHTML = '';
  _stepsMap.clear(); _stepsId = 0;
...
function addStep(text, state, sub, meta) {
  ...
  if (meta && meta.code) {
    ...
    txtEl.innerHTML = text.replace(/</g, '&lt;') +
      '<svg width="10" height="10" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-width="2" style="flex-shrink:0;color:var(--cyan);"><polyline points="9 18 15 12 9 6"/></svg>';
  ...
  var cnt = document.getElementById('stepsCount');
  if (cnt) {
    var done = _stepsList.querySelectorAll('[data-st="done"]').length;
    cnt.textContent = '(' + done + '/' + _stepsId + ')';
  }
  return id;
}

function updateStep(id, state, text, sub) {
  var row = _stepsMap.get(id); if (!row) return;
  row.setAttribute('data-st', state);
  var ic = row.querySelector('.step-ic');
  if (ic) {
  ...
  if (text) { var t = row.querySelector('.step-txt'); if (t) t.textContent = text; }
  if (sub !== undefined) {
    var sv = row.querySelector('.step-sub');
...
function finalizeSteps() {
  var spinner = document.getElementById('stepsSpinner'); if (spinner) spinner.style.display = 'none';
  var hdrTxt = document.getElementById('stepsTxt'); if (hdrTxt) hdrTxt.style.color = 'var(--green)';
  var cancelDiv = document.getElementById('stepsCancel'); if (cancelDiv) cancelDiv.remove();
  _stepsMap.forEach(function (row) {
...
// ── MENTION ───────────────────────────────────────────────────────────────────
async function fetchWsCache() {
  if (_wsCache || _wsLoading || !SESSION || !studioConnected) return;
  _wsLoading = true;
  try {
    var ctrl = new AbortController();
    setTimeout(function () { ctrl.abort(); }, 8000);
    var r = await fetch(API_URL + '?get_workspace=1&user=' + encodeURIComponent((SESSION.user.username || '').toLowerCase()), { signal: ctrl.signal });
    if (r.ok) {
      var d = await r.json();
      if (d && d.data) _wsCache = d.data;
      else if (d && (d.Workspace || d.ServerScriptService)) _wsCache = d;
    }
  } catch (e) { }
  _wsLoading = false;
}

function extractMentionItems(scan, query) {
  if (!scan) return [];
  var items = [], q = (query || '').toLowerCase();
  function traverse(node, svc) {
    if (!node || !node.name || items.length >= 25) return;
    if (!q || node.name.toLowerCase().includes(q)) items.push({ name: node.name, cls: node.class || '', svc: svc });
    if (node.children) node.children.forEach(function (c) { traverse(c, svc); });
  }
  ['ServerScriptService', 'ReplicatedStorage', 'StarterGui', 'StarterPlayer', 'StarterPack', 'ReplicatedFirst', 'Workspace'].forEach(function (sn) {
    if (scan[sn] && scan[sn].children) scan[sn].children.forEach(function (c) { traverse(c, sn); });
    else if (scan[sn]) traverse(scan[sn], sn);
  });
  items.sort(function (a, b) {
    return (a.cls.includes('Script') ? 0 : 1) - (b.cls.includes('Script') ? 0 : 1) || a.name.localeCompare(b.name);
  });
...
function showMentionDD(query) {
  var dd = document.getElementById('mentionDD'), inp = document.getElementById('inp');
  if (!dd || !inp) return;
  var items = extractMentionItems(_wsCache, query);
  var list = document.getElementById('mentionList');
  if (!list) return;
  _mentionSelIdx = 0;
  if (!studioConnected) {
    list.innerHTML = '<div class="mention-empty">' + (curLang === 'id' ? 'Studio belum terhubung' : 'Studio not connected') + '</div>';
  } else if (!_wsCache) {
    list.innerHTML = '<div class="mention-empty">' + (curLang === 'id' ? 'Memuat...' : 'Loading...') + '</div>';
    fetchWsCache();
  } else if (!items.length) {
    list.innerHTML = '<div class="mention-empty">' + (curLang === 'id' ? 'Tidak ada hasil' : 'No results') + '</div>';
  } else {
    list.innerHTML = items.map(function (item, idx) {
      var ic = getMentionIcon(item.cls);
      return '<div class="mention-item' + (idx === 0 ? ' sel' : '') + '" onclick="insertMention(\'' + esc(item.name) + '\')">' +
        '<div class="mention-ic ' + ic.css + '">' + ic.lbl + '</div>' +
        '<div style="flex:1;min-width:0;"><div class="mention-name">' + esc(item.name) + '</div>' +
        '<div class="mention-path">' + esc(item.cls || 'Instance') + ' \u2014 ' + esc(item.svc) + '</div></div></div>';
    }).join('');
  }
...
function hideMentionDD() {
  var dd = document.getElementById('mentionDD'); if (dd) dd.classList.remove('open');
...
function insertMention(name) {
  ...
  var atPos = _mentionAtPos >= 0 ? _mentionAtPos : val.lastIndexOf('@', pos - 1);
  if (atPos >= 0) {
...
function moveMentionSel(dir) {
  var list = document.getElementById('mentionList'); if (!list) return;
  var items = list.querySelectorAll('.mention-item');
  if (!items.length) return;
  _mentionSelIdx = Math.max(0, Math.min(items.length - 1, _mentionSelIdx + dir));
  items.forEach(function (el, i) { el.classList.toggle('sel', i === _mentionSelIdx); });
  if (items[_mentionSelIdx]) items[_mentionSelIdx].scrollIntoView({ block: 'nearest' });
}

function selectCurrentMention() {
  var list = document.getElementById('mentionList'); if (!list) return false;
  var sel = list.querySelectorAll('.mention-item')[_mentionSelIdx];
  if (!sel) return false;
...
function handleFile(e) {
  ...
  files.forEach(function (file) {
    if (file.type.startsWith('image/')) {
      ...
      var reader = new FileReader();
      reader.onload = function (ev) {
        S.attachments.push({ type: 'file', name: file.name, text: ev.target.result });
        renderAttachRow();
      };
  ...
  for (var i = 0; i < items.length; i++) {
    if (items[i].type.indexOf('image') !== -1) {
      ...
      if (file) {
        ...
        reader.onload = function (ev) {
          var d = ev.target.result;
          S.attachments.push({ type: 'image', name: 'pasted.png', mime: 'image/png', data: d.split(',')[1], preview: d });
          renderAttachRow();
...
if (_inpBox) {
  _inpBox.addEventListener('dragover', function (e) { e.preventDefault(); this.classList.add('drag-over'); });
  _inpBox.addEventListener('dragleave', function () { this.classList.remove('drag-over'); });
...
function openAvatarModal() {
  ...
  if (imgEl) { imgEl.src = u.avatar || _NEXUS_ICON_SVG; imgEl.onerror = function () { this.src = _NEXUS_ICON_SVG; }; }
...
function switchTab(tab, btn) {
  ...
  if (tab === 'chat') { ct.style.display = 'flex'; ct.style.flexDirection = 'column'; gt.style.display = 'none'; }
  else { ct.style.display = 'none'; gt.style.display = 'flex'; gt.style.flexDirection = 'column'; }
}

function likeMsg(btn, isLike) {
  var msgEl = btn.closest('.msg.ai'); if (!msgEl) return;
...
function openShareModal() {
  ...
  var ta = document.getElementById('shareModalTa'); if (ta) ta.value = text;
...
async function sendReport() {
  ...
  if (K.turnstile && window.turnstile && _turnstileWidget !== null) {
    try {
      cfToken = await new Promise(function (res, rej) {
        ...
        turnstile.getResponse(_turnstileWidget) ? (res(turnstile.getResponse(_turnstileWidget)), clearTimeout(tid)) : rej('no token');
      });
    } catch (e) {
      try {
  ...
  try {
    await fetch(REPORT_URL, {
      ...
      body: JSON.stringify({ from: SESSION && SESSION.user && SESSION.user.username || '?', userId: SESSION && SESSION.user && SESSION.user.robloxId || '?', message: ta.value, type: 'report', 'cf-turnstile-response': cfToken, time: new Date().toISOString() })
    });
...
async function redeemCode() {
  ...
  if (btn) btn.disabled = true;
  try {
    var r = await fetch('/api/redeem', { method: 'POST', headers: getAdminHeaders(), body: JSON.stringify({ code: code, user: (SESSION && SESSION.user && SESSION.user.username || '').toLowerCase(), userId: SESSION && SESSION.user && SESSION.user.robloxId || '' }) });
    var d = await r.json();
    if (d.success) { S.credits += parseFloat(d.credits || 0); updateCreds(); saveS(); if (st) st.textContent = '+' + d.credits + ' CR'; if (inp) inp.value = ''; }
...
function applyGuiTheme(themeName) {
  ...
  Object.keys(guiElements).forEach(function (id) { guiElements[id].bgColor = th.panel; guiElements[id].textColor = th.text; renderGuiEl(id); });
...
function addEl(type) {
  ...
  guiElements[id] = Object.assign({}, def, { id: id, type: type, name: type + '_' + guiElCounter, x: 20 + guiElCounter * 12, y: 20 + guiElCounter * 10 });
  var empt = document.getElementById('guiEmpty'); if (empt) empt.style.display = 'none';
  renderGuiEl(id); selectEl(id); updateLayerList();
}

function renderGuiEl(id) {
  var el = guiElements[id]; if (!el) return;
  var canvas = document.getElementById('guiCanvasInner'); if (!canvas) return;
  var existing = canvas.querySelector('[data-elid="' + id + '"]'); if (existing) existing.remove();
  var div = document.createElement('div');
  div.className = 'gui-el'; div.setAttribute('data-elid', id);
  ...
  if (el.text) { var sp = document.createElement('span'); sp.textContent = el.text; sp.style.cssText = 'pointer-events:none;padding:0 4px;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100%;'; div.appendChild(sp); }
  div.onmousedown = function (e) { startDrag(e, id); };
  var resize = document.createElement('div'); resize.className = 'gui-resize';
  resize.onmousedown = function (e) { startResize(e, id); };
  div.appendChild(resize); canvas.appendChild(div);
}

function updateLayerList() {
  var list = document.getElementById('guiLayerList'); if (!list) return;
  var els = Object.values(guiElements);
  if (!els.length) { list.innerHTML = ''; return; }
  var typeColors = { Frame: '#00e5ff', TextLabel: '#00ffaa', TextButton: '#ff4fa0', TextBox: '#ffd600', ImageLabel: '#cc55ff', ScrollingFrame: '#8800ff' };
...
function selectEl(id) {
  ...
  var el = document.querySelector('[data-elid="' + id + '"]'); if (el) el.classList.add('selected');
  updatePropsPanel(); updateLayerList();
}

function updatePropsPanel() {
  var p = document.getElementById('guiProps'); if (!p) return;
  var t = T();
  if (!selectedElId || !guiElements[selectedElId]) {
    p.innerHTML = '<div style="font-size:10px;color:var(--dim);text-align:center;padding:20px 0;">' + t.guiPropsEmpty + '</div>'; return;
  }
...
function updateElProp(elId, prop, val) {
  if (!guiElements[elId]) return; guiElements[elId][prop] = val;
  var el = document.querySelector('[data-elid="' + elId + '"]'); if (!el) return;
  if (prop === 'text') { var sp = el.querySelector('span'); if (sp) sp.textContent = val; else if (val) { var s2 = document.createElement('span'); s2.textContent = val; s2.style.cssText = 'pointer-events:none;padding:0 4px;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100%;'; el.appendChild(s2); } }
...
function generateGuiCode() {
  ...
  els.forEach(function (el) {
    ...
    if (el.type !== 'Frame' && el.type !== 'ScrollingFrame' && el.type !== 'ImageLabel') { if (el.textColor) lines.push(v + '.TextColor3 = Color3.fromRGB(' + hx(el.textColor) + ')'); lines.push(v + '.Text = "' + String(el.text || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '"'); if (el.fontSize) lines.push(v + '.TextSize = ' + el.fontSize); lines.push(v + '.Font = Enum.Font.GothamBold'); lines.push(v + '.TextXAlignment = Enum.TextXAlignment.Center'); }
  ...
  var out = document.getElementById('guiCodeOutput'); if (out) out.textContent = lines.join('\n');
...
async function generateGuiFromAI() {
  ...
  var loading = document.getElementById('guiLoading'); if (loading) loading.classList.add('show');
  ...
  try {
    ...
    var body = { provider: S.guiModel.prov || 'gemini', model: S.guiModel.id, system: sysMsg, messages: [{ role: 'user', content: 'Create: ' + prompt.value }], max_tokens: 3000 };
    ...
    if (r.ok) {
      ...
      if (jm) {
        try {
          ...
          var gts = document.getElementById('guiThemeSelect'); if (gts) gts.value = theme;
          parsed.forEach(function (el) {
            if (!el.type) return; guiElCounter++;
            var id = 'el' + guiElCounter;
            guiElements[id] = Object.assign({ id: id, cornerRadius: 0, fontSize: 14, text: '', bgColor: th.panel, textColor: th.text }, el, { w: Math.max(el.w || 200, 40), h: Math.max(el.h || 40, 20), x: Math.max(el.x || 0, 0), y: Math.max(el.y || 0, 0) });
            renderGuiEl(id);
          });
          var empt = document.getElementById('guiEmpty'); if (empt) empt.style.display = 'none';
          updateLayerList();
          toast(curLang === 'id' ? 'UI berhasil dibuat!' : 'UI built successfully!', 'var(--green)', 2500);
        } catch (e) { toast(t.aiResponseInvalid, 'var(--yellow)'); }
      } else { toast(t.aiResponseInvalid, 'var(--yellow)'); }
    } else { toast('API error: ' + r.status, 'var(--pink)'); }
  } catch (e) { toast('Error: ' + (e && e.message || 'unknown'), 'var(--pink)'); }
...
async function sendGuiToPlace() {
  ...
  try {
    var r = await fetchRetry(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'batch_commands', commands: [cmd], _user: SESSION ? SESSION.user.username : 'web', target: (SESSION ? SESSION.user.username : '').toLowerCase() }) }, 3);
    var d = await r.json();
    if (d.pushed > 0 || d.status === 'ok') toast(t.guiSentToast, 'var(--green)');
    else toast(curLang === 'id' ? 'Diantri ke Studio' : 'Queued to Studio', 'var(--yellow)');
...
function startDrag(e, elId) {
  ...
  function onMove(ev) { var nx = Math.max(0, sl + ev.clientX - sx), ny = Math.max(0, st2 + ev.clientY - sy); el.style.left = nx + 'px'; el.style.top = ny + 'px'; if (guiElements[elId]) { guiElements[elId].x = nx; guiElements[elId].y = ny; } }
  function onUp() { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); updatePropsPanel(); }
...
function startResize(e, elId) {
  ...
  function onMove(ev) { var nw = Math.max(40, sw + ev.clientX - sx), nh = Math.max(20, sh + ev.clientY - sy); el.style.width = nw + 'px'; el.style.height = nh + 'px'; if (guiElements[elId]) { guiElements[elId].w = nw; guiElements[elId].h = nh; } }
  function onUp() { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); updatePropsPanel(); }
  document.addEventListener('mousemove', onMove); document.addEventListener('mouseup', onUp);
}
...
function toggleThemeDD(e) {
  e.stopPropagation(); var dd = document.getElementById('themeDD'); if (!dd) return;
...
function selectTheme(themeName) {
  ...
  S.selectedTheme = themeName; var isCustom = themeName === 'custom';
...
// FIX: Handle visibility change - refresh studio status when tab becomes visible
