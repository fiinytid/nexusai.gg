// lib/inbox.ts — NEXUS AI Inbox System (SECURE v3 — TypeScript)
//
// Changes v3 (JS → TS):
//   • Full TypeScript strict types — no implicit 'any'
//   • InboxMessage, InboxStore interfaces menggantikan objek anonim
//   • AdaptedRequest / AdaptedResponse dari route.ts digunakan konsisten
//   • Import verifyAdminToken, sanitizeStr, checkRateLimit dari _security.ts
//   • Semua fungsi helper dianotasi penuh
//   • Tidak ada perubahan behaviour — semua logic identik dengan v2
...
type InboxStore = Record<string, InboxMessage[]>;
...
const INBOX_FILE        = '/tmp/nexus_inbox.json' as const;
const MAX_MSGS_PER_USER = 50;
const MAX_CONTENT_LEN   = 3_000;

// ─── STORAGE ──────────────────────────────────────────────────────────────────

function getInbox(): InboxStore {
  try {
    if (existsSync(INBOX_FILE)) {
      return JSON.parse(readFileSync(INBOX_FILE, 'utf8')) as InboxStore;
    }
  } catch { /* silently return empty */ }
  return {};
}

function saveInbox(d: InboxStore): void {
  try { writeFileSync(INBOX_FILE, JSON.stringify(d)); } catch { /* silently fail */ }
}
...
const handler: HandlerFn = async (req: AdaptedRequest, res: AdaptedResponse) => {
  ...
  if (req.method === 'POST') {
    if (!verifyAdminToken(req)) {
      return res.status(403).json({
        error: 'Forbidden: Admin token diperlukan untuk mengirim pesan.',
      });
    }
  ...
  if (req.method === 'DELETE') {
    ...
    // Deleting messages requires admin token; marking read is self-service
    const isDeletion = action === 'delete' || (!id && !action);
    if (isDeletion && !verifyAdminToken(req)) {
      return res.status(403).json({
        error: 'Forbidden: Admin token diperlukan untuk menghapus pesan.',
      });
    }
