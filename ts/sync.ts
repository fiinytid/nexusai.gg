...
//     Previously, `trimMsgs()` truncated any message content longer than
//     6,000 characters and stripped `_rawContent` entirely before persisting
//     to Supabase/KV. That meant long Lua scripts generated by the AI were
//     saved in a SHORTENED form, and reopening a chat later showed the
//     truncated version instead of the original. This is now fixed:
//     messages are stored with their FULL content and FULL `_rawContent`,
//     so reopening a conversation later shows exactly what was generated.
//   • REMOVED destructive attachment stripping. Previously, image
//     attachments were rewritten to `{ type: 'image', name, mime }` before
//     saving — the actual base64 image `data` was thrown away, so photos
//     disappeared on reload. Attachments (including base64 image data) are
//     now persisted as-is in Supabase.
//   • REPLACED the old "trim everyone's conversations the same way" payload
//     guard with a smarter, non-destructive-to-recent-data strategy:
//     when (and only when) a payload would exceed the configured size
//     limit, we drop whole OLDER conversations (oldest-first) — never
//     truncate the *content* of a conversation, and never touch the most
//     recent conversations. This keeps the most relevant/recent chats
//     (including their full code and full images) intact, while still
//     protecting Supabase/KV from oversized-row errors (HTTP 413 / write
//     failures). The byte-size guard is now PER-PAYLOAD (computed from the
//     actual JSON about to be written), not a blanket character limit
//     applied to every message regardless of actual size.
//   • Kept all integrated redeem-code management actions:
...
//   • No changes to existing sync / CRUD / storage logic beyond what's listed above
